...
※1:エリア外を検出して前進・後進を抑制するのはアバターコア側の責務の想定  
...
|要求|備考|
|:---|:---|
|Miimo Main ECUは帰還コマンドを受け付けた場合、ドッキング移動中に遷移し、エリアセンサ信号から左右走行モータ駆動信号を算出する|※1※2※3|
|Miimo Main ECUはリリースコマンドを受け付けた場合、リリース待ちに遷移する|※1※4|
|Miimo Main ECUはリリース待ち状態で安全確認操作を受け付けた場合、エリアセンサ信号から左右走行モータ駆動信号を算出する|※2※3※4|
|Miimo Main ECUはキャンセルコマンドの受け付け/自動運転の完了/自動運転のタイムアウトにより、リリース待ち無し/遠隔操作モードに遷移する|※1※5|
|Miimo Main ECUは走行操作コマンドを受け付けた場合、左右走行モータへのモータ駆動信号に変換する|※6|
|右走行モータはモータ駆動信号を受け取り動作する|※7|
|左走行モータはモータ駆動信号を受け取り動作する|※7|

※1:機能コマンドはアバターコア側で要求モードに変換したものを受け取る想定  
※2:充電ステーション近傍での自動運転を想定（近傍以外の自動運転はアバターコア側に指示責務があるものと想定）  
※3:「エリアセンサ信号から左右走行モータ駆動信号を算出する」は自律走行中/リリース動作準備にて実施される想定  
※4:リリースの自動運転は、リリースコマンド受付→安全確認→自動運転開始となる想定。安全確認操作は電源スイッチ短押しの想定  
※5:自動運転の完了については、ドッキングはDC-ON検知、リリースはDC-OFF検知し一定距離（20cmを想定）後進することで自動運転完了となる想定。タイムアウトについては、自動運転開始から一定時間内に終了しなかった場合に発生する想定（タイムアウト時間はリリース自動運転とドッキング自動運転で異なる想定。具体的な時間はTBD）。キャンセルコマンドの受け付け/自動運転の完了/自動運転のタイムアウトの時点で、DC-ONであればリリース待ち無し、DC-ON以外であれば遠隔操作モードに遷移する想定  
※6:走行操作コマンドはアバターコア側で並進速度とヨーレート指令に変換したものを受け取る想定  
※7:機能干渉マトリクスより、走行モータ駆動中に別のモータ駆動信号が入力された場合、その信号に従って動作する  
