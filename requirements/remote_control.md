...
〇:コマンド受付可 / ×:コマンド受付不可  
※:帰還コマンドは速度0の時のみ受付可  
状態の詳細は「テレプレゼンスロボット要求分析_L0L1要求抽出.docx」内の「状態遷移図」>「SM-02:動作中」/「SM-02-1:ステーション待機中」参照
...
|Miimo Main ECUはキャンセルコマンドを受け付けた場合、動作モードをドッキング/リリース動作モードから解除する|※1|
|Miimo Main ECUは走行操作コマンドを受け付けた場合、左右走行モータへのモータ駆動信号に変換する|※4|
|右走行モータはモータ駆動信号を受け取り動作する|※5|
|左走行モータはモータ駆動信号を受け取り動作する|※5|
...
※4:走行操作コマンドはアバターコア側で並進速度とヨーレート指令に変換したものを受け取る想定  
※5:機能干渉マトリクスより、走行モータ駆動中に別のモータ駆動信号が入力された場合、その信号に従って動作する  
