...
|UC-010 バッテリ低下時に一時停止し、通知する|バッテリ低下時に一時停止し、その旨をユーザーに通知する|・ユーザー|
...
ユースケース「バッテリ低下時に一時停止し、通知する」におけるブロック間のインタラクションを以下に示す
...
- 事前条件
	- 起動処理（アバタークラウドへの接続）が完了したテレプレゼンスロボットが存在すること
- 事後条件
	- テレプレゼンスロボットが一時停止し、アバターアプリに通知が送られていること
- 不変条件
	- なし
- 代替フロー
    - なし
...
|テレプレゼンスロボットはバッテリ残量が低下したら、一時停止する||
|テレプレゼンスロボットはバッテリ残量低下により一時停止したことを通知する||
...
|状態|定義|備考|
|:---|:---|:---|
|電源OFF|テレプレゼンスロボットに電源が入っていない状態（充電端子に接続されていない）|SM-01参照|
|通電中|テレプレゼンスロボットに電源が入っていない状態（充電端子に接続されている）|充電中、充電完了の表示は行う想定<br/>SM-01、SM-02参照|
|電源ON|テレプレゼンスロボットの電源が入っている状態|頭部・胴体部が起動している状態を想定<br/>SM-01、SM-03参照|
|起動中|電源ON状態の中で起動処理を行っている状態|SM-03参照|
|動作中|電源ON状態の中で起動処理が終わった状態|Fault/Warning/Caution状態を含む<br/>SM-03、SM-04参照|
|正常動作中|動作中状態の中でFault/Warning/Caution状態ではない状態|SM-04、SM-05参照|
|Fault|動作中状態の中で内部デバイスの破損により走行ができない状態|SM-04参照|
|Warning1|動作中状態の中でバッテリ残量低下により走行ができない状態|バッテリ残量低下<br/>SM-04、SM-08参照|
|Warning2|動作中状態の中で異常検知し走行ができない状態|CAN通信異常など<br/>SM-04、SM-08参照|
|Caution|動作中状態の中で保護機能が動作し、走行動作を停止している状態|SM-04参照|
|DC-ON|正常動作中状態の中で充電端子に接続されている状態|SM-05、SM-06参照|
|リリース待ち無し|DC-ON状態の中でリリース動作を受け付けていない状態|SM-06参照|
|リリース待ち1|DC-ON状態の中でリリースコマンドを受け付けた状態|リリースコマンドは頭部からの指示を想定（リリース開始指示後、リリース動作を開始する）<br/>SM-06参照|
|リリース待ち2|DC-ON状態の中でリリース開始指示を受け付けた状態|リリース開始指示は電源スイッチ短押しの想定（リリースコマンド受付後、リリース動作を開始する）<br/>リリース開始指示は一定期間内にリリースコマンドが無い場合、タイムアウトする想定<br/>SM-06参照|
|リリース中(DC-ON)|DC-ON状態の中でリリース動作を行っている状態|SM-06参照|
|DC-OFF|正常動作中状態の中で充電端子に接続されていない状態|SM-05、SM-07参照|
|遠隔操作（外部指示）|DC-OFF状態の中で外部から遠隔操作が行える状態|頭部からCAN通信で走行部を制御できる状態を想定<br/>SM-07参照|
|一時停止|DC-OFF状態の中で（TODO）|SM-07参照|
|ドッキング中|DC-OFF状態の中でドッキング指示によりドッキング動作を行っている状態|SM-07参照|
|リリース中(DC-OFF)|DC-OFF状態の中でリリース動作を行っている状態|SM-07参照|
|終了中|電源ON状態の中で終了処理を行っている状態|SM-03参照|
|充電中|DC-ON状態または通電中状態の中でバッテリーを充電している状態|SM-02、SM-06参照|
|充電停止|DC-ON状態または通電中状態の中でバッテリーの充電が完了している状態|SM-02、SM-06参照|
|異常|異常を検出している状態|（TODO）Fault、Warning1、Warning2、Cautionを包含|
...
尚、状態遷移より導出される要求については影響を受けるUCで個別に抽出を行う

<!-- TODO:状態遷移図に番号をつける-->

|No.|状態遷移|トリガー|影響を受けるUC|備考|
|---:|:---|:---|:---|:---|
|1|電源OFF → 通電中|充電ステーションに接続|SysRS-16|SM-01、SM-02参照|
|2|電源OFF → 電源ON|電源ON操作|SysRS-01|SM-01、SM-03参照<br/>電源ON操作は電源スイッチ短押し、スイッチ長押し操作を想定|
|3|通電中 → 電源ON|電源ON操作|SysRS-01,SysRS-16|SM-01、SM-02参照<br/>電源ON操作は電源スイッチ短押し操作のみを想定|
|4|電源ON → 電源OFF|電源OFF操作[充電端子未接続]|SysRS-15|SM-01、SM-03参照<br/>電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|5|電源ON → 通電中|電源OFF操作[充電端子接続]|SysRS-15,SysRS-16|SM-01、SM-03参照<br/>電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|6|正常動作中 → Fault|Fault検知|SysRS-11|SM-04参照|
|7|正常動作中 → Warning1|バッテリ残量低下検知|SysRS-11|SM-04、SM-08参照|
|8|正常動作中 → Warning2|その他Warning検知|SysRS-11|SM-04、SM-08参照<br/>Warning1が優先|
|9|正常動作中 → Caution|Caution検知|SysRS-11|SM-04参照|
|9|Warning → 正常動作中|異常復帰操作|(TODO)|SM-04参照|
|9|Caution → 正常動作中|異常復帰操作|(TODO)|SM-04参照|
|10|DC-OFF → DC-ON|充電端子接続|SysRS-16|SM-05参照|
|11|DC-ON → DC-OFF|充電端子接続解除|SysRS-16|SM-05参照|
|12|リリース待ち無し → リリース待ち1|リリースコマンド受付|SysRS-03|SM-06参照|
|13|リリース待ち無し → リリース待ち2|リリース開始指示受付|SysRS-03|SM-06参照<br/>リリース開始指示は電源スイッチ短押しの想定|
|14|リリース待ち1 → リリース中(DC-ON)|リリース開始指示受付|SysRS-03|SM-06参照<br/>リリース開始指示は電源スイッチ短押しの想定|
|15|リリース待ち1 → リリース待ち無し|キャンセルコマンド受付|SysRS-03|SM-06参照<br/>タイムアウトはしない想定|
|16|リリース待ち2 → リリース中(DC-ON)|リリースコマンド受付|SysRS-03|SM-06参照|
|17|リリース待ち2 → リリース待ち無し|リリース開始解除指示受付 or タイムアウト|SysRS-03|SM-06参照<br/>リリース開始解除指示は電源スイッチ短押しの想定|
|18|リリース中(DC-ON) → リリース待ち無し|リリース開始解除指示 or キャンセルコマンド受付|SysRS-03|SM-06参照<br/>リリース開始解除指示は電源スイッチ短押しの想定<br/>実際にあり得る遷移か要QA|
|19|リリース中(DC-ON) → リリース中(DC-OFF)|DC-OFF検知|SysRS-03|SM-06、SM-07参照<br/>実際にあり得る遷移か要QA|
|20|リリース中(DC-OFF) → 遠隔操作中|キャンセルコマンド受付 or 自動遷移|SysRS-03|SM-06、SM-07参照|
|21|DC-ON → 遠隔操作中|充電端子接続解除|SysRS-16|SM-06、SM-07参照|
|22|遠隔操作中 → 一時停止中|一時停止操作|SysRS-06|SM-07参照<br/>一時停止操作は電源スイッチ短押しの想定|
|23|リリース中(DC-OFF) → 一時停止中|一時停止操作|SysRS-06|SM-07参照<br/>一時停止操作は電源スイッチ短押しの想定|
|24|一時停止中 → 遠隔操作中|一時停止解除操作|SysRS-07|SM-07参照<br/>一時停止解除操作は電源スイッチ短押しの想定|
|25|遠隔操作中 → ドッキング中|帰還コマンド受付|SysRS-06|SM-03参照|
|26|ドッキング中 → 遠隔操作中|キャンセルコマンド受付|SysRS-06|SM-03参照|
|27|ドッキング中 → 一時停止中|一時停止操作|SysRS-06|SM-03参照<br/>一時停止操作は電源スイッチ短押しの想定|
|28|充電中 → 充電完了|満充電検知|SysRS-16|SM-02、SM-06参照<br/>SM-06ではリリース待ち状態優先|
|29|充電完了 → 充電中|充電量低下検知|SysRS-16|SM-02、SM-06参照<br/>SM-06ではリリース待ち状態優先|
|30|Warning1 → Warning2|バッテリ残量回復検知|SysRS-11|SM-08参照|
|31|Warning2 → Warning1|バッテリ残量低下検知|SysRS-11|SM-08参照|
|NN|XXX → YYY|TRG-01|SysRS-xxx||
...
## SysRS-10:バッテリ残量低下を検知し一時停止する

**ユースケース概要**

テレプレゼンスロボットのバッテリ残量低下を検知し一時停止する
...
- 事後条件
    ...
    - バッテリー残量低下により一時停止していることがアバタークラウドに通知されていること
- 備考
