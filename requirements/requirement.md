...
|状態|定義|備考|
|:---|:---|:---|
|電源OFF|テレプレゼンスロボットに電源が入っていない状態||
|電源ON|テレプレゼンスロボットの電源が入っている状態||
|起動中|電源ON操作により起動処理を行っている状態||
|DC-OFF|充電端子に接続されていない状態||
|遠隔操作（外部指示）|外部から遠隔操作が行える状態|頭部からCAN通信で走行部を制御できる状態を想定|
|ドッキング動作|ドッキング指示によりドッキング動作を行っている状態||
|DC-ON|充電端子に接続されている状態||
|充電中|DC-ON状態でバッテリーを充電している状態||
|充電停止|DC-ON状態でバッテリーの充電が完了している状態||
|リリース待ち|DC-ON状態でリリース動作指示を持っている状態||
|リリース動作|リリース動作を行っている状態||
|一時停止|（TODO）||
|終了|電源OFF操作により終了処理を行っている状態||
|異常|異常を検出している状態|Fault、Warning1、Warning2、Cautionを包含|
|Fault|内部デバイスの破損により走行ができない状態||
|Warning1|（TODO）|バッテリ残量低下|
|Warning2|（TODO）|CAN通信異常など|
|Caution|保護機能が動作し、走行動作を停止している状態||
|リプロ|リプロモードで起動している状態||
|テスト|テストモードで起動している状態||
...
![](.images/statemachine/st02.png)

**異常状態**

![](.images/statemachine/st03.png)
...
|No.|状態遷移|トリガー|影響を受けるUC|
|---:|:---|:---|:---|
|1|電源OFF → DC-ON|充電ステーションに接続|SysRS-01|
|2|DC-ON → 電源ON|電源ON操作|SysRS-01|
|3|DC-ON → リリース待ち|リリース指示|SysRS-03,|
|n|AA → BB|TRG01|SysRS-xx|
|m|CC → DD|TRG02|SysRS-xx|
