...
ユースケース「異常を検知して停止する」に次の2つのインタラクション（シナリオ）がある、それぞれのブロック間のインタラクションを以下に示す
...
・異常の有り/無し  
を直交状態と考え、整理した。  
直交状態として表現した状態遷移図を以下に示す

![](.images/statemachine/st_all_orthogonal.png)

<div style="page-break-before:always"></div>

以下に直交状態を合成した状態についての概要を示す。

|状態|電源|充電ステーション|異常有無|備考|
|:---|:---|:---|:---|:---|
|ST01_正常運転|ON|未接続|無し|※1|
|ST02_異常|ON|未接続|有り|※2|
|ST03_充電中|ON|接続|無し||
|ST04_充電中異常|ON|接続|有り|※3※7|
|ST05_電源OFF|OFF|未接続|無し|※4|
|ST06_電源OFF異常|OFF|未接続|有り|※4|
|ST07_通電中|OFF|接続|無し||
|ST08_通電中異常|OFF|接続|有り|※3※5|
|ST09_テスト|ON|---|---|※6|

※1:リプロ状態は起動条件（電源ON/充電ステーション未接続/異常無し）よりST01_正常運転の特殊状態として分析する  
※2:Warning1の異常の場合、ドッキングが異常復帰操作を兼ねるため、ST03_充電中に遷移する  
※3:Warning1となることは無い  
※4:外部から見てST_電源OFFとST06_電源OFF異常は区別がつかない（異常有無の状態遷移以外で事後的に観測される）  
※5:異常復帰操作は電源ON操作を兼ねるため、ST03_充電中に遷移する  
※6:テスト状態は電源ON状態の特殊状態と考えられるが、他の状態（充電ステーション接続/未接続、異常有無）に依らず内部処理は変わらないため、まとめてST09_テストとして分析する  
※7:頭部の異常により頭部がOFFとなった場合（Faultとなる想定）も含む想定

<div style="page-break-before:always"></div>

ここで、依存関係のある状態を確認するため、
以下のような直交状態を合成した状態遷移図を作成し分析を実施した。

![](.images/statemachine/st_all_orthogonal_composite.png)

<div style="page-break-before:always"></div>

上記分析を踏まえて、内部状態のST01_正常運転～ST08_通電中異常をさらに詳細化し分析を行った。  

<div style="page-break-before:always"></div>

### 状態遷移図（内部状態）

抽出された内部状態の状態遷移図を以下に記載する

**ST01_正常運転**

![](.images/statemachine/sub_ST01.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す
|状態|定義|備考|
|:---|:---|:---|
|ST01_正常運転|電源ONされ充電ステーション未接続で異常の無い状態|システム全体が起動している想定|
|起動中|起動処理を実施している状態|システム全体の起動を行う想定。|
|動作中|起動処理が完了し動作可能な状態|さらに内部状態として、遠隔操作不可、遠隔操作可能を包含する|
|遠隔操作不可|動作中状態内で遠隔操作が出来ない状態|一時停止中を想定|
|遠隔操作可能|動作中状態内で遠隔操作可能な状態|さらに内部状態として、遠隔操作待ち、走行操作中、自律走行中を包含する|
|遠隔操作待ち|遠隔操作可能状態内で速度0により停止している状態||
|走行操作中|遠隔操作可能状態内で走行操作（速度0ではない）されている状態||
|自律走行中|遠隔操作可能状態内で自律走行している状態|自律走行としてはドッキング動作中とリリース動作中がある想定|
|終了中|終了処理を実施している状態|システム全体の終了を行う想定|
|リプロ|リプロモードとなっている状態|ソフトウェアアップデート等で使用される特殊状態を想定。モード終了後は、終了中を経てST05_電源OFFとなる想定|


本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガ|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST01_TR-01|ST05_電源OFF → 起動中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定|
|ST01_TR-02|ST03_充電中 → 動作中（遠隔操作不可）|手動リリース|SysRS-06,SysRS-16|自動リリースをトリガとする遷移はST01_TR-04参照|
|ST01_TR-03|ST02_異常 → 動作中（遠隔操作不可）|異常復帰操作|SysRS-06,SysRS-11b|異常復帰操作は電源スイッチ短押しを想定|
|ST01_TR-04|ST03_充電中（自律走行中） → 動作中（遠隔操作可能（自律走行中））|DC-OFF検知|SysRS-03,SysRS-16|リリースの自動走行中にDC-OFFを検知した場合の振る舞い|
|ST01_TR-05|動作中 → ST03_充電中（動作中）|ドッキング|SysRS-03,SysRS-16|起動中は動作中に遷移後のドッキング、終了中はST05_電源OFFに遷移後のドッキングと同じ振る舞いと想定し省略|
|ST01_TR-06|動作中 → ST02_異常（動作中）|異常検知|SysRS-11a|起動中は動作中に遷移後の異常検知、終了中はST05_電源OFFに遷移後の異常検知と同じ振る舞いと想定し省略|
|ST01_TR-07|動作中/リプロ/ST09_テスト → 終了中|電源OFF操作|SysRS-15|終了後はST05_電源OFFに自動遷移を想定|
|ST01_TR-08|遠隔操作不可 → 遠隔操作可能|遠隔操作可能指示|SysRS-03,SysRS-07|遠隔操作可能指示は電源スイッチ短押しを想定|
|ST01_TR-09|遠隔操作可能 → 遠隔操作不可|遠隔操作禁止指示|SysRS-03,SysRS-06|遠隔操作禁止指示は電源スイッチ短押しを想定|
|ST01_TR-10|遠隔操作待ち/走行操作中 → 走行操作中|走行操作指示|SysRS-03|走行操作指示はアバタークラウドからの前進/後進/旋回指示を想定|
|ST01_TR-11|走行操作中 → 遠隔操作待ち|速度0検知|SysRS-03|走行操作指示がされず、速度0となった場合に遠隔操作待ちとなることを想定|
|ST01_TR-12|遠隔操作待ち → 自律走行中|自律走行指示|SysRS-03|自律走行指示はアバタークラウドからの充電ステーションへの帰還指示を想定|
|ST01_TR-13|自律走行中 → 遠隔操作待ち|キャンセル指示/自動リリースの完了|SysRS-03|※1|
|ST01_TR-14|遠隔操作不可 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|
|ST01_TR-15|遠隔操作不可 → リプロ|リプロモード遷移指示|SysRS-22|リプロモード遷移指示は書き換え用PCから受信する想定|

※1:キャンセル指示はアバタークラウドからの充電ステーションへの帰還指示キャンセルおよび充電ステーションからのリリース指示キャンセルを想定。リリース指示による自律走行中の完了に伴う自動遷移によっても遠隔操作待ちとなる想定。遠隔操作禁止指示（電源スイッチ短押しを想定）はST01_TR-09となる想定  
...
![](.images/statemachine/sub_ST02.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST02_異常|電源ONされ充電ステーション未接続で異常のある状態|システム全体が起動（異常により頭部がOFFである場合も含む）している想定|
|起動中|起動処理を実施している状態|システム全体の起動を行う想定|
|動作中|起動処理が完了し動作可能な状態|ここでは遠隔操作を受け付けない想定|
|終了中|終了処理を実施している状態|システム全体の終了を行う想定|
|Fault|Faultレベルの異常が発生している状態||
|Warning1|Warning1レベルの異常が発生している状態|ドッキングにより異常から正常に復帰する想定|
|復帰操作可能異常|復帰操作により正常に復帰できる状態|復帰操作は電源スイッチ短押しを想定。さらに内部状態として、Warning2、Cautionを包含する|
|Warning2|復帰操作可能異常状態内でWarning2レベルの異常が発生している状態||
|Caution|復帰操作可能異常状態内でCautionレベルの異常が発生している状態||

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST02_TR-01|ST06_電源OFF異常 → 起動中|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|電源ON操作は電源スイッチ短押し/長押しを想定|
|ST02_TR-02|ST04_充電中異常 → 動作中|リリース|SysRS-16||
|ST02_TR-03|ST01_正常運転 → 動作中|異常検知|SysRS-11a||
|ST02_TR-04|動作中/ST09_テスト → 終了中|電源OFF操作|SysRS-11b,SysRS-15|※1|
|ST02_TR-05|動作中 → ST04_充電中異常|ドッキング[Warning1を除く]|SysRS-16||
|ST02_TR-06|動作中 → ST03_充電中|ドッキング[Warning1]|SysRS-11b,SysRS-16|Warning1発生時はドッキングにより異常から復帰する想定|
|ST02_TR-07|動作中 → ST01_正常運転|異常復帰操作[復帰操作可能異常]|SysRS-06,SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。Fault、Warning1は異常復帰操作は受け付けない想定|
|ST02_TR-08|動作中 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|

※1:電源OFF操作は電源スイッチ長押し/アバターコアからのシャットダウン操作を想定。終了中後の自動遷移によりST06_電源OFF異常となる想定  

<div style="page-break-before:always"></div>

**ST03_充電中**

![](.images/statemachine/sub_ST03.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST03_充電中|電源ONされ充電ステーション接続中で異常の無い状態|システム全体が起動している想定|
|起動中|起動処理を実施している状態|システム全体の起動を行う想定|
|動作中|起動処理が完了し動作可能な状態|さらに内部状態として、リリース待ち無し、リリース待ち、自律走行中を包含する|
|終了中|終了処理を実施している状態|システム全体（通電中でも動作する部分を除く）の終了を行う想定|
|リリース待ち無し|動作中状態内でリリース指示を受け付け可能な状態|充電ステーション接続状態の内部状態が状態表示される|
|リリース待ち|動作中状態内で自律走行の開始を待っている状態|自律走行は安全確認された後に開始する想定|
|自律走行中|動作中状態内で自律走行している状態|自律走行としては充電ステーションからのリリースを想定|
|充電中|バッテリが充電中である状態||
|充電完了|バッテリの充電が完了している状態||

本状態に関する内部状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST03_TR-01|ST07_通電中 → 起動中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押しを想定（電源スイッチ長押しは無視される想定）|
|ST03_TR-02|ST01_正常運転 → 動作中|ドッキング|SysRS-03,SysRS-16||
|ST03_TR-03|ST04_充電中異常 → 動作中|異常復帰操作|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定|
|ST03_TR-04|動作中 → ST01_正常運転|手動リリース|SysRS-06,SysRS-16|※1|
|ST03_TR-05|動作中 → ST04_充電中異常|異常検知|SysRS-11a|※2|
|ST03_TR-06|自律走行中 → ST01_正常運転（自律走行中）|DC-OFF検知|SysRS-03,SysRS-16|リリースの自動走行中にDC-OFFを検知した場合の振る舞い|
|ST03_TR-07|動作中/ST09_テスト → 終了中|電源OFF操作|SysRS-15|電源OFF操作は電源スイッチ長押しを想定。終了中後はST07_通電中に自動遷移を想定|
|ST03_TR-08|リリース待ち無し → リリース待ち|自律走行指示|SysRS-03|自律走行指示はアバタークラウドからの充電ステーションからのリリース指示を想定|
|ST03_TR-09|リリース待ち → リリース待ち無し|自律走行取消指示|SysRS-03|自律走行取消指示はアバタークラウドからのキャンセルコマンドを想定|
|ST03_TR-10|リリース待ち → 自律走行中|安全確認操作|SysRS-03|安全確認操作は電源スイッチ短押しを想定|
|ST03_TR-11|自律走行中 → リリース待ち無し|自律走行取消指示、安全確認取消操作|SysRS-03|※3|
|ST03_TR-12|充電中 → 充電完了|満充電検知|SysRS-16|満充電検知はバッテリ充電100%を想定|
|ST03_TR-13|充電完了 → 充電中|充電不足検知|SysRS-16|充電不足検知はバッテリ充電が閾値以下（具体値はTBD)を想定|
|ST03_TR-14|ST02_異常 → 動作中|ドッキング[Warning1]|SysRS-11b,SysRS-16|Warning1発生時はドッキングにより異常から復帰する想定|
|ST03_TR-15|リリース待ち無し → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|

※1:起動中は動作中に遷移後のリリース、終了中はST01_正常運転に遷移後のリリースと同じ振る舞いと想定し省略。自動リリースについてはST03_TR-06参照  
※2:起動中は動作中に遷移後の異常検知、終了中はST01_正常運転に遷移後の異常検知と同じ振る舞いと想定し省略  
※3:自律走行取消指示ははアバタークラウドからのキャンセルコマンドを想定、安全確認取消操作は電源スイッチ短押しを想定  

<div style="page-break-before:always"></div>

**ST04_充電中異常**

![](.images/statemachine/sub_ST04.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST04_充電中異常|電源ONされ充電ステーション接続中で異常のある状態|システム全体が起動（異常により頭部がOFFである場合も含む）している想定|
|起動中|起動処理を実施している状態|システム全体の起動を行う想定|
|動作中|起動処理が完了し動作可能な状態|ここでは遠隔操作を受け付けない想定|
|終了中|終了処理を実施している状態|システム全体（通電中でも動作する部分を除く）の終了を行う想定|
|Fault|Faultレベルの異常が発生している状態||
...
|Warning2|復帰操作可能異常状態内でWarning2レベルの異常が発生している状態||
|Caution|復帰操作可能異常状態内でCautionレベルの異常が発生している状態||
※:ST04_充電中異常においてWarning1レベルの異常は発生しない想定

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST04_TR-01|ST08_通電中異常 → 起動中|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|※1|
|ST04_TR-02|ST02_異常 → 動作中|ドッキング|SysRS-16||
|ST04_TR-03|ST03_充電中 → 動作中|異常検知|SysRS-11a||
|ST04_TR-04|動作中/ST09_テスト → 終了中|電源OFF操作|SysRS-11b,SysRS-15|※2|
|ST04_TR-05|動作中 → ST03_充電中|異常復帰操作[復帰操作可能異常]|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。Faultは異常復帰操作は受け付けない想定|
|ST04_TR-06|動作中 → ST02_異常|リリース|SysRS-16||
|ST04_TR-07|動作中 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|

※1:QA結果（No.80/81）よりST08_通電中異常での電源ON操作は異常復帰操作を兼ねるため、異常が解消された場合、ST03_充電中に遷移する  
※2:電源OFF操作は電源スイッチ長押し/アバターコアからのシャットダウン操作を想定。終了中後の自動遷移によりST08_通電中異常となる想定  

<div style="page-break-before:always"></div>

**ST05_電源OFF**

![](.images/statemachine/sub_ST05.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST05_電源OFF|電源OFFで充電ステーション未接続で正常な状態|ST06_電源OFF異常と外部観測で区別できない。電源ON操作やドッキングにより遷移した先の違いにより事後的に判別される|

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST05_TR-01|ST05_電源OFF → ST01_正常運転|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定|
|ST05_TR-02|ST05_電源OFF → ST07_通電中|ドッキング|SysRS-16||
|ST05_TR-03|ST05_電源OFF → ST06_電源OFF異常|異常検知||外部観測不能な遷移|

<div style="page-break-before:always"></div>

**ST06_電源OFF異常**

![](.images/statemachine/sub_ST06.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST06_電源OFF異常|電源OFFで充電ステーション未接続で異常が発生している状態|ST05_電源OFFと外部観測で区別できない。電源ON操作やドッキングにより遷移した先の違いにより事後的に判別される|

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST06_TR-01|ST06_電源OFF異常 → ST02_異常|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|電源ON操作は電源スイッチ短押し/長押しを想定※1|
|ST06_TR-02|ST06_電源OFF異常 → ST08_通電中異常|ドッキング|SysRS-16||
|ST06_TR-03|ST06_電源OFF異常 → ST05_電源OFF|異常復帰操作||外部観測不能な遷移|

※1:電源ON操作により異常が解消された場合は、ST01_正常運転に遷移する

<div style="page-break-before:always"></div>

**ST07_通電中**

![](.images/statemachine/sub_ST07.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST07_通電中|電源OFFで充電ステーション接続され正常な状態|異常検知し状態表示可能なように部分的に電源ONされる想定|
|充電中|バッテリが充電中である状態||
|充電完了|バッテリの充電が完了している状態||
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST07_TR-01|ST07_通電中 → ST03_充電中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押しを想定（電源スイッチ長押しは何もしない想定）|
|ST07_TR-02|ST07_通電中 → ST05_電源OFF|リリース|SysRS-15,SysRS-16|リリースは手動リリースのみ|
|ST07_TR-03|ST07_通電中 → ST08_通電中異常|異常検知|SysRS-11a||
|ST07_TR-04|充電中 → 充電完了|満充電検知|SysRS-16|満充電検知はバッテリ充電100%を想定|
|ST07_TR-05|充電完了 → 充電中|充電不足検知|SysRS-16|充電不足検知はバッテリ充電が閾値以下（具体値はTBD)を想定|

<div style="page-break-before:always"></div>

**ST08_通電中異常**

![](.images/statemachine/sub_ST08.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST08_通電中異常|電源OFFで充電ステーション接続され異常のある状態|異常検知し状態表示可能なように部分的に電源ONされる想定|
|Fault|Faultレベルの異常が発生している状態||
|復帰操作可能異常|復帰操作により正常に復帰できる状態|復帰操作は電源スイッチ短押しを想定。さらに内部状態として、Warning2、Cautionを包含する|
|Warning2|復帰操作可能異常状態内でWarning2レベルの異常が発生している状態||
|Caution|復帰操作可能異常状態内でCautionレベルの異常が発生している状態||

※:ST08_通電中異常においてWarning1レベルの異常は発生しない想定  

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST08_TR-01|ST08_通電中異常 → ST03_充電中|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|※1|
|ST08_TR-02|ST08_通電中異常 → ST06_電源OFF異常|リリース|SysRS-15,SysRS-16|※2|
|ST08_TR-03|ST08_通電中異常 → ST03_充電中|異常復帰操作|SysRS-11b|※1|

※1:QA結果（No.80/81）より電源ON操作(電源スイッチ短押しを想定)/異常復帰操作（電源スイッチ短押し/長押しを想定）は電源ONと異常復帰を兼ねる操作となりST03_充電中への遷移となる（異常解消できなければ、ST04_充電中異常への遷移となる）想定  
※2:QA結果（No.80/81）よりFault発生時はリリースにより充電ステーション未接続の電源OFF状態とした後の再起動でのみ正常状態での起動を試行可能な想定

<div style="page-break-before:always"></div>

**ST09_テスト**

![](.images/statemachine/sub_ST09.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|ST09_テスト|テストモードとなっている状態|電源ON状態内の特殊状態|

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST09_TR-01|ST01_正常運転/ST02_異常/ST03_充電中/ST04_充電中異常 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|
|ST09_TR-02|ST09_テスト → (ST01_正常運転/ST02_異常/ST03_充電中/ST04_充電中異常)終了中|電源OFF操作|SysRS-15|テスト終了時は、終了中を経て電源OFF状態となる想定|
...
- 備考
    ...
    - ST05_TR-01(ST01_TR-01)、ST06_TR-01(ST02_TR-01)より、
      - ST05_電源OFF/ST06_電源OFF異常から充電ステーションに接続された場合は、ロボットの一部のみ起動する（充電中/充電完了/Fault/Warning2/Cautionの状態表示する（起動中の状態表示しない））
        - ロボットの一部はMiimo Main ECUと表示LEDの起動を想定
    - ST07_TR-01(ST03_TR-01)、ST08_TR-01(ST04_TR-01)より
      - ST07_通電中に電源ON操作をされた場合は、ロボット全体を起動する（起動中の状態表示する）
        - 上記電源ON操作は電源スイッチ短押し操作である（電源スイッチ長押し操作は受け付けない）想定
        - QA結果（No.80/81）よりST08_通電中異常では、電源スイッチ短押し/長押し操作が電源ON操作と異常復帰操作を兼ねるため、ST03_充電中への遷移となる
...
- 備考
	- 本ユースケース（UC）の実行中に、以下の各UCも実行される。詳細については各UCの記述を参照のこと
        ...
        - バッテリ残量低下を検知し停止する
        - 異常を検知し停止する
    - ST01_TR-08、ST01_TR-09より
      - 遠隔操作可能/不可の切り替えは一時停止/一時停止解除操作による想定
    - ST01_TR-05(ST03_TR-02)、ST01_TR-10、ST01_TR-11、ST01_TR-12、ST01_TR-13より
      - ST01_正常運転>動作中>遠隔操作可能において
        - 遠隔操作待ちでは走行操作指示（走行操作コマンド）または自律走行開始指示（帰還コマンド）を受け付ける
        - 走行操作指示を受付後、走行操作中状態となる
        - 自律走行開始指示を受付後、帰還動作を開始する（自律走行中に遷移する）とする
      - 走行操作中では、速度0検知まで走行操作指示のみ受け付ける
      - 自律走行中では、キャンセル指示のみ受け付ける
        - キャンセル指示を受付後、遠隔操作待ちとなる
        - 自律走行がリリース動作であった場合、リリース動作完了後、遠隔操作待ちに自動遷移する
        - 自律走行がドッキング動作であった場合、ドッキング動作完了=TR01_TR-05により、ST03_充電中>動作中に遷移する
    - ST01_TR-04(ST03_TR-06)、ST03_TR-08、ST03_TR-09、ST03_TR-10、ST03_TR-11より
      - ST03_充電中>動作中において
        - リリース待ち無しでは自律走行開始指示（リリースコマンド）のみを受け付ける、受付後、リリース待ち（状態表示はリリース待ち）になる
        - リリース待ちでは安全確認操作（電源スイッチ短押し）か自動走行取消指示（キャンセルコマンド）を受け付ける。
          - 安全確認操作を受付後、リリース動作を開始する（自律走行中に遷移する)とする
          - 自律走行取消指示を受付後、リリース待ち無しに戻る（状態表示は、充電中か充電完了）とする
        - 自律走行中（DC-OFF検知前）では自動走行取消指示または安全確認取消操作（電源スイッチ短押し）を受け付け、リリース待ち無しに戻る（状態表示は、充電中か充電完了）とする
        - 自律走行中でDC-OFF検知するとST01_正常運転>動作中>遠隔操作可能>自律走行中に遷移する
...
- 備考
    - ST01_TR-09より
      - ST01_正常運転では、一時停止操作（状態遷移分析では遠隔操作禁止操作と表記）により一時停止中（状態遷移分析では遠隔操作不可状態と表記）になるものとする。
        - 一時停止操作は電源スイッチ短押し操作である想定
    - ST01_TR-02(ST03_TR-04)、ST01_TR-03(ST02_TR-07)より
      - ST03_充電中からの手動リリース、ST02_異常からの異常復帰操作後は一時停止中となるものとする
...
- 備考
	- ST01_TR-08により
    	- 一時停止状態(状態遷移分析内ではST01_正常運転>遠隔操作不可)での一時停止解除操作（状態遷移分析では遠隔操作可能指示）はST01_正常運転>遠隔操作可能>遠隔操作待ちとなるものとする
...
- 備考
    - ST01_TR-06(ST02_TR-03)、ST03_TR-05(ST04_TR-03)、ST07_TR-03より
      - 充電ステーション接続中（ST04_充電中異常、ST08_通電中異常）はWarning1があり得ない想定
      - 異常の判定の優先順位は高い方から、Fault→Warning1→Warning2→Cautionの想定
    - ST02_TR-01(ST06_TR-01)、ST04_TR-01(ST08_TR-01)より
      - 異常発生中の電源ON操作時は、異常復帰処理が試行される想定（そのため、正常状態へ遷移する場合もある）
...
- 備考
	- テレプレゼンスロボットを再起動してもFault状態が解消されない場合はメンテナンスが必要
	- ST01_TR-03(ST02_TR-07)、ST03_TR-03(ST04_TR-05)より
    	- Warning2、Caution状態では電源スイッチ短押しによる復帰操作を受け付ける想定
    	- Fault、Warning1は電源スイッチ短押しによる復帰操作を受け付けない
  	- ST02_TR-04、ST04_TR-04より
    	- Faultは電源OFF操作（電源スイッチ長押し/シャットダウン操作）を行い、ST06_電源OFF異常となった後に、電源ON操作（電源スイッチ短押しおよび充電ステーション未接続であれば電源スイッチ長押しも可）を行うことで正常状態への復帰を想定
  	- ST02_TR-06(ST03_TR-14)より
    	- Warning1はドッキングすることにより正常状態（ST03_充電中またはST07_通電中）に遷移する
  	- ST06_TR-01(ST02_TR-01)より
    	- 電源ON操作により異常が解消された場合はST01_正常運転に遷移する想定
  	- ST08_TR-01(ST04_TR-01)、ST08_TR-03より
    	- QA結果（No.80/81）よりST08_通電中異常状態では電源スイッチ短押し/長押しともに異常復帰操作となり、ST03_充電中（異常が解消されていなければST04_充電中異常）に遷移する想定
...
- 事後条件
    ...
    - 電源OFFとなること
- 備考
    - ST01_TR-07、ST02_TR-04より
      - 電源スイッチ長押し/シャットダウン操作により終了状態を経てST05_電源OFF/ST06_電源OFF異常に遷移する想定
      - 終了処理はシステム全体を終了する想定
    - ST03_TR-07、ST04_TR-04より
      - 電源スイッチ長押し/シャットダウン操作により終了状態を経てST07_通電中/ST08_通電中異常に遷移する想定
      - 終了処理は通電中でも動作する部分を除いたシステム全体を終了する想定
    - ST07_TR-02、ST08_TR-02より
      - ST07_通電中/ST08_通電中異常で手動リリースされた場合、ロボット全体を終了する（状態表示は終了中表示しない）
        - Miimo Main ECUの終了処理を想定 
...
- 事後条件
    - 充電を停止する
- 備考
    - ST01_TR-05(ST03_TR-02)、ST02_TR-05(ST04_TR-02)、ST02_TR-06(ST03_TR-02)、ST03_TR-12、ST03_TR-13、ST03_TR-14、ST05_TR-02、ST06_TR-02、ST07_TR-04、ST07_TR-05より
      - 充電ステーション接続中は、異常有無、リリース待ち有無に関わらず、内部的には充電中か充電完了が制御されている
      - ST04_充電中異常/ST08_通電中異常であれば、異常状態を優先して表示する
      - ST03_充電中の場合、リリース待ち状態を優先して表示する
      - ST07_通電中の場合、充電中/充電完了状態を表示する
      - ST02_異常でWarning1異常発生時に充電ステーション接続された場合は、異常から復帰し正常状態に遷移する（ST06_電源OFF異常でWarning1異常発生の場合は、ST05_電源OFFに遷移するが外部観測はできない。ST04_充電中異常、ST08_通電中異常ではWarning1が発生しない）
    - ST01_TR-02(ST03_TR-04)、ST01_TR-04(ST03_TR-06)、ST02_TR-02(ST04_TR-06)、ST07_TR-02、ST08_TR-02より
      - 充電ステーション切り離し時は、電源ON/OFF・異常有無に応じた遷移先に遷移する
...
- 備考
	- ST09_TR-01(ST01_TR-14/ST02_TR-08/ST03_TR-15/ST04_TR-07)より
    	- テストモード遷移指示を診断PCから受信することによりテストモードとなる想定
  	- ST09_TR-02(ST01_TR-07/ST02_TR-04/ST03_TR-07/ST04_TR-04)より
    	- テストモード終了後は、電源OFF操作により終了中を経て電源OFF状態に遷移する想定
...
- 備考
	...
	- ST01_TR-15より
    	- リプロモード遷移指示を書き換え用PCから受信することによりリプロモードとなる想定
  	- ST01_TR-07より
    	- リプロモード終了後は、電源OFF操作により終了中を経てST05_電源OFFに遷移する想定
