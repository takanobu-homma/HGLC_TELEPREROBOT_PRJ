...
|Warning|異常を検知し、走行を停止|バッテリ異常、モーター異常|復帰ボタンを操作|
...
|---:|:---|:---|:---|:---|
|ST-01|電源OFF → 通電中|手動ドッキング|SysRS-01<br/>SysRS-16|SM-02参照|
|ST-02|通電中 → 電源OFF|手動リリース|SysRS-15<br/>SysRS-16|SM-02参照|
|ST-03|電源OFF → 頭部電源ON|電源ON操作|SysRS-01|SM-03参照<br/>電源ON操作は電源スイッチ短押し、スイッチ長押し操作を想定|
|ST-04|頭部電源ON → 電源OFF|電源OFF操作[充電ステーション未接続]|SysRS-15|SM-03参照<br/>電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|ST-05|異常停止中 → 電源OFF|電源OFF操作[充電ステーション未接続]|SysRS-11<br/>SysRS-15|SM-08参照<br/>電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|ST-06|異常停止中 → 通電中|電源OFF操作[充電ステーション接続]|SysRS-11<br/>SysRS-15<br/>SysRS-16|SM-08参照<br/>充電ステーション接続時は頭部電源ONであれば、電源スイッチ長押し、シャットダウン操作で通電中になる。頭部電源OFFであれば、電源スイッチ長押しは無視される想定|
|ST-07|一時停止中 → テスト|テスト開始(TODO)|SysRS-19<br/>SysRS-20<br/>SysRS-21|SM-07参照<br/>遷移条件、内部状態等は(TODO)|
|ST-08|テスト → 一時停止中|(TODO)|SysRS-19<br/>SysRS-20<br/>SysRS-21|SM-07参照<br/>遷移条件、内部状態等は(TODO)|
|ST-09|一時停止中 → リプロ|リプロ開始(TODO)|SysRS-22|SM-07参照<br/>遷移条件、内部状態等は(TODO)|
|ST-10|リプロ → 一時停止中|(TODO)|SysRS-22|SM-07参照<br/>遷移条件、内部状態等は(TODO)|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-11|頭部電源ON → 通電中|電源OFF操作[充電ステーション接続]|SysRS-15<br/>SysRS-16|電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|ST-12|通電中 → 頭部電源ON|電源ON操作|SysRS-01<br/>SysRS-16|電源ON操作は電源スイッチ短押し操作を想定|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-13|充電中 → 充電完了|満充電検知|SysRS-16|SM-06参照<br/>SM-06ではリリース待ち状態優先|
|ST-14|充電完了 → 充電中|充電量低下検知|SysRS-16|SM-06参照<br/>SM-06ではリリース待ち状態優先|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-15|正常動作中 → Fault|Fault検知|SysRS-11|SM-08参照|
|ST-16|正常動作中 → Warning1|バッテリ残量低下検知|SysRS-10|SM-08参照|
|ST-17|正常動作中 → Warning2|その他Warning検知|SysRS-11|SM-08参照<br/>Warning1が優先|
|ST-18|正常動作中 → Caution|Caution検知|SysRS-11|SM-08参照|
|ST-19|Warning1 → 正常動作中|異常復帰操作|SysRS-10|SM-09参照<br/>異常復帰操作は充電ステーションへの接続の想定|
|ST-20|Warning2 → 正常動作中|異常復帰操作|SysRS-11|SM-09参照<br/>異常復帰操作は電源スイッチ短押しの想定|
|ST-21|Caution → 正常動作中|異常復帰操作|SysRS-11|SM-08参照<br/>異常復帰操作は電源スイッチ短押しの想定|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-22|DC-OFF → DC-ON|充電端子接続|SysRS-16||
|ST-23|DC-ON → DC-OFF|充電端子接続解除|SysRS-16||
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-24|リリース待ち無し → リリース待ち1|リリースコマンド受付|SysRS-03<br/>SysRS-16||
|ST-25|リリース待ち無し → リリース待ち2|リリース開始指示受付|SysRS-03<br/>SysRS-16|リリース開始指示は電源スイッチ短押しの想定|
|ST-26|リリース待ち1 → リリース中(DC-ON)|リリース開始指示受付|SysRS-03<br/>SysRS-16|リリース開始指示は電源スイッチ短押しの想定|
|ST-27|リリース待ち1 → リリース待ち無し|キャンセルコマンド受付|SysRS-03<br/>SysRS-16|タイムアウトはしない想定|
|ST-28|リリース待ち2 → リリース中(DC-ON)|リリースコマンド受付|SysRS-03<br/>SysRS-16||
|ST-29|リリース待ち2 → リリース待ち無し|リリース開始解除指示受付 or タイムアウト|SysRS-03<br/>SysRS-16|リリース開始解除指示は電源スイッチ短押しの想定|
|ST-30|リリース中(DC-ON) → リリース待ち無し|リリース開始解除指示 or キャンセルコマンド受付|SysRS-03<br/>SysRS-16|リリース開始解除指示は電源スイッチ短押しの想定<br/>実際にあり得る遷移か要QA|
|ST-31|リリース中(DC-ON) → リリース中(DC-OFF)|DC-OFF検知|SysRS-03|SM-07参照<br/>実際にあり得る遷移か要QA|
|ST-32|リリース中(DC-OFF) → 遠隔操作中|キャンセルコマンド受付 or 自動遷移|SysRS-03|SM-07参照|
|ST-33|DC-ON → 一時停止中|手動リリース|SysRS-16|SM-07参照|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-34|遠隔操作中 → 一時停止中|一時停止操作|SysRS-06|一時停止操作は電源スイッチ短押しの想定|
|ST-35|リリース中(DC-OFF) → 一時停止中|一時停止操作|SysRS-03<br/>SysRS-06|一時停止操作は電源スイッチ短押しの想定|
|ST-36|一時停止中 → 遠隔操作中|一時停止解除操作|SysRS-07|一時停止解除操作は電源スイッチ短押しの想定|
|ST-37|遠隔操作中 → ドッキング中|帰還コマンド受付|SysRS-03<br/>SysRS-06||
|ST-38|ドッキング中 → 遠隔操作中|キャンセルコマンド受付|SysRS-03||
|ST-39|ドッキング中 → 一時停止中|一時停止操作|SysRS-03<br/>SysRS-06|一時停止操作は電源スイッチ短押しの想定|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|ST-40|Warning1 → Warning2|バッテリ残量回復検知|SysRS-10<br/>SysRS-11<br/>SysRS-16||
|ST-41|Warning2 → Warning1|バッテリ残量低下検知|SysRS-10<br/>SysRS-11<br/>SysRS-16||
...
- 備考
    ...
    - ST-01より
      - 電源OFF状態から充電ステーションに接続された場合は、ロボットの一部のみ起動する（通電中の状態表示する（起動中の状態表示しない））
        - ロボットの一部はMiimo Main ECUと表示LEDの起動を想定
    - ST-12より
      - 通電中状態で、電源ON操作をされた場合は、ロボット全体を起動する（起動中の状態表示する）
...
- 備考
	- 本ユースケース（UC）の実行中に、以下の各UCも実行される。詳細については各UCの記述を参照のこと
        ...
        - バッテリ残量低下を検知し停止する
        - 異常を検知し停止する
    - ST-24、ST-25、ST-26、ST-27、ST-28、ST-29より
      - DC-ON状態（充電ステーションに接続して頭部電源ONの状態）場合にて受け付けるリリースコマンドは、リリース指示受付操作およびリリースコマンドの二つを受けてリリース動作を開始する（リリース中（DC-ON)状態に遷移する)とする
      - 上記リリース指示受付操作、リリースコマンドの一方を受けた場合、リリース待ち状態（状態表示はリリース待ち）になる
      - 上記リリース指示受付操作によるリリース待ち状態は再度リリース指示解除操作によりリリース待ち無し状態（状態表示は充電中か充電完了）に戻る
      - 上記リリース指示受付操作によるリリース待ち状態はタイムアウトによりリリース待ち無し状態（状態表示は充電中か充電完了）に戻る
      - 上記リリースコマンドによるリリース待ち状態はキャンセルコマンドによりリリース待ち無し状態（状態表示は充電中か充電完了）に戻る（タイムアウトはしない）
        - 上記リリース指示受付操作、リリース指示解除操作はどちらも電源スイッチ短押し操作である想定
    - ST-30、ST-31、ST-32より
      - リリース動作中でDC-ON（リリース中(DC-ON)状態）の場合、キャンセルコマンドまたはリリース開始停止操作により、リリース待ち無し状態に戻る。
      - リリース動作中でDC-OFF（リリース中(DC-OFF)状態）の場合、キャンセルコマンドにより遠隔操作中状態となる。一時停止操作により一時停止中状態となる。
        - リリース開始停止操作、一時停止操作はどちらも電源スイッチ短押し操作である想定
...
- 備考
    - ST-34、ST-35、ST-37、ST-39より
      - DC-OFF状態（充電ステーションから離れている電源ON状態）場合、一時停止操作により一時停止中状態になるものとする。
...
- 備考
	- ST-36により
    	- 一時停止状態での一時停止解除操作は遠隔操作中状態となるものとする
...
- 備考
    - ST-15、ST-17、ST-18、ST-20、ST-21より
      - バッテリ残量低下以外の警告（Warning2状態）は異常復帰操作により解消される（充電ステーション接続しても状態表示は変わらない）
        - 異常復帰操作は電源スイッチ短押し操作である想定
    - ST-05、ST-06より
      - 異常停止中の電源OFF操作は異常無しの電源OFF操作と変わらない挙動
      - 充電ステーション接続中で頭部電源OFFでのFault状態は、手動リリース後の電源ON操作でのみ解消される（充電ステーション接続中で頭部電源がOFFであれば、電源OFF操作は無視される）
        - 電源OFF操作は電源スイッチ長押し、頭部からのシャットダウン操作である想定
    - ST-40、ST-41より
      - バッテリ残量低下による警告が解消されて他のWarningがあった場合は、他のWarningがあることを状態表示する。
      - 他のWarningがありバッテリ残量低下による警告が発生した場合は、バッテリ残量低下による警告が発生したことを状態表示する。
...
|要求|備考|
|:---|:---|
|頭部は走行部からの指示により異常（CautionおよびWarning）状態を解消する||
|頭部は異常状態が解消されたことを通知する||
|胴体部はCautionおよびWarning1状態の時に復帰操作を受け付ける||
|胴体部は異常状態が解消されたことを表示する||
|走行部はWarning1状態の時に充電ステーションへの接続による復帰操作を受け付ける||
|走行部は異常（CautionおよびWarning）状態を解消する||
|走行部は異常（CautionおよびWarning）状態を解消されたことを確認する||
...
- 備考
    - ST-02より
      - 通電中（充電ステーション接続中で頭部電源OFF）状態で手動リリースされた場合、ロボット全体を終了する（状態表示は終了中表示しない）
        - Miimo Main ECUの終了処理を想定
    - ST-04、ST-11より
      - 頭部電源ON状態での電源OFF操作は<br/>・充電ステーション未接続なら、電源OFFする（状態表示は終了中表示する）<br/>・充電ステーション接続中なら、頭部を電源OFF（シャットダウン処理）する（状態表示は終了中表示する）
    - ST-05、ST-06より
      - 異常停止中状態での電源OFF操作は上記異常無し状態の電源OFF操作と同様の挙動となる。そのため、充電ステーション接続中で頭部電源OFFでのFault状態は、手動リリース後の電源ON操作でのみ解消される（充電ステーション接続中で頭部電源OFFでの電源OFF操作は無視される）。
...
- 備考
    - ST-01より
      - 電源OFF状態から充電ステーションに接続された場合は、ロボットの一部のみ起動する（通電中の状態表示する（起動中の状態表示しない））
        - ロボットの一部はMiimo Main ECUと表示LEDの起動を想定
    - ST-02より
      - 通電中（充電ステーション接続中で頭部電源OFF）状態で手動リリースされた場合、ロボット全体を終了する（終了中の状態表示しない）
        - Miimo Main ECUの終了処理を想定
    - ST-11より
      - 充電ステーション接続中の頭部電源ON状態での電源OFF操作は頭部を電源OFF（シャットダウン処理）する（終了中の状態表示する）
    - ST-12より
      - 通電中状態で、電源ON操作をされた場合は、頭部（首モータ含む）を起動する（起動中の状態表示する）
        - 電源ON操作は電源スイッチ短押し操作を想定（電源スイッチ長押し操作ではない想定）
    - ST-06より
      - ・充電ステーション接続中で異常停止中の電源OFF操作は頭部電源ONでのみ有効。頭部電源OFFとなる。（充電ステーション接続中で頭部電源OFFでの電源OFF操作は無視される）。
        - ここでの電源OFF操作は電源スイッチ長押し操作を想定。
    - ST-24、ST-25、ST-26、ST-27、ST-28、ST-29より
      - リリース待ち状態表示は、充電中・充電完了の表示よりも優先して表示される
