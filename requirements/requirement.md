...
ここで、直交状態として表現したもののうち、依存関係のある状態を確認するため、
...
|状態|電源|充電ステーション|異常有無|定義|備考|
|:---|:---|:---|:---|:---|:---|
|State1|ON|未接続|無し|電源ONで充電ステーションから外れており正常な状態<br/>外部から見るとアバターコアの画面が有効で一時停止か遠隔操作により走行している（遠隔操作による速度0/自律走行中も含む）|ユーザ使用中のメイン状態|
|State2|ON|未接続|有り|電源ONで充電ステーションから外れており異常がある状態<br/>外部から見ると充電ステーションから外れてFault/Warning1/Warning2/Cautionのいずれかにより停止している|Faultに関しては、異常復帰操作は電源OFF操作と同じため、State1への遷移はしない|
|State3|ON|接続|無し|電源ONで充電ステーションに接続されており正常な状態<br/>外部から見るとアバターコアの画面が有効でリリース待ち/充電中/充電完了のいずれかで充電ステーションに接続している||
|State4|ON|接続|有り|電源ONで充電ステーションに接続されており異常がある状態<br/>外部から見るとFault/Warning2/Cautionのいずれかで充電ステーションに接続している|Warning1となることは無い<br/>Faultに関しては、異常復帰操作は電源OFF操作と同じため、State3への遷移はしない|
|State5|OFF|未接続|無し|電源OFFで充電ステーションから外れており正常な状態<br/>外部から見るとアバターコアの画面が無効で充電ステーションから外れて停止しているように見える|外部から見てState6と区別はつかない（異常有無の状態遷移以外で事後的に観測される）|
|State6|OFF|未接続|有り|電源OFFで充電ステーションから外れており正常な状態<br/>外部から見るとアバターコアの画面が無効で充電ステーションから外れて停止しているように見える|外部から見てState5と区別はつかない（異常有無の状態遷移以外で事後的に観測される）|
|State7|OFF|未接続|無し|電源OFFで充電ステーションに接続されており正常な状態<br/>外部から見るとアバターコアの画面が無効で充電中/充電完了のいずれかで充電ステーションに接続している||
|State8|OFF|未接続|有り|電源OFFで充電ステーションに接続されており異常がある状態<br/>外部から見るとアバターコアの画面が無効でFault/Warning2/Cautionのいずれかで充電ステーションに接続している|Warning1となることは無い<br/>Faultに関しては、異常復帰操作は電源OFF操作と同じため、State7への遷移はしない|

<div style="page-break-before:always"></div>

上記分析を踏まえて、内部状態のState1～State8をさらに詳細化し分析を行った。  
...
**State1_電源ON_DC-OFF_正常**

![](.images/statemachine/State1_電源ON_DC-OFF_正常.png)
...
|DC-OFF|充電ステーション未接続であることを示す状態||
|正常|異常が無いことを示す状態||

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガ|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State1_TR-01|State5_電源OFF_DC-OFF_正常 → 起動中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定。State5_TR-01と等価|
|State1_TR-02|State3_電源ON_DC-ON_正常 → 動作中（遠隔操作不可）|手動リリース|SysRS-16|State3_TR-04と等価。自動リリースをトリガとする遷移はState1_TR-04参照|
|State1_TR-03|State2_電源ON_DC-OFF_異常 → 動作中（遠隔操作不可）|異常復帰操作|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。State2_TR-07と等価|
|State1_TR-04|State3_電源ON_DC-ON_正常（自律走行中） → 動作中（遠隔操作可能（自律走行中））|DC-OFF検知|SysRS-03|リリースの自動走行中にDC-OFFを検知した場合の振る舞い。State3_TR-06と等価|
|State1_TR-05|動作中 → State3_電源ON_DC-ON_正常（動作中）|ドッキング|SysRS-03, SysRS-16|起動中は動作中に遷移後のドッキング、終了中はState5_電源OFF_DC-OFF_正常に遷移後のドッキングと同じ振る舞いと想定し省略。State3_TR-02と等価|
|State1_TR-06|動作中 → State2_電源ON_DC-OFF_異常（動作中）|異常検知|SysRS-11a|起動中は動作中に遷移後の異常検知、終了中はState5_電源OFF_DC-OFF_正常に遷移後の異常検知と同じ振る舞いと想定し省略。State2_TR-03と等価|
|State1_TR-07|動作中 → 終了中|電源OFF操作|SysRS-15|終了後はState5_電源OFF_DC-OFF_正常に自動遷移を想定|
|State1_TR-08|遠隔操作不可 → 遠隔操作可能|遠隔操作可能指示|SysRS-03,SysRS-07|遠隔操作可能指示は電源スイッチ短押しを想定|
|State1_TR-09|遠隔操作可能 → 遠隔操作不可|遠隔操作禁止指示|SysRS-03,SysRS-06|遠隔操作禁止指示は電源スイッチ短押しを想定|
|State1_TR-10|遠隔操作待ち/走行操作中 → 走行操作中|走行操作指示|SysRS-03|走行操作指示はアバタークラウドからの前進/後進/旋回指示を想定|
|State1_TR-11|走行操作中 → 遠隔操作待ち|速度0検知|SysRS-03|走行操作指示がされず、速度0となった場合に遠隔操作待ちとなることを想定|
|State1_TR-12|遠隔操作待ち → 自律走行中|自律走行指示|SysRS-03|自律走行指示はアバタークラウドからの充電ステーションへの帰還指示を想定|
|State1_TR-13|自律走行中 → 遠隔操作待ち|キャンセル指示/自動リリースの完了|SysRS-03|キャンセル指示はアバタークラウドからの充電ステーションへの帰還指示キャンセルおよび充電ステーションからのリリース指示キャンセルを想定。リリース指示による自律走行中の完了に伴う自動遷移によっても遠隔操作待ちとなる想定。遠隔操作禁止指示（電源スイッチ短押しを想定）はState1_TR-09となる想定|

<div style="page-break-before:always"></div>

**State2_電源ON_DC-OFF_異常**

![](.images/statemachine/State2_電源ON_DC-OFF_異常.png)
...
|---:|:---|:---|:---|:---|
|State2_TR-01|State6_電源OFF_DC-OFF_異常 → 起動中|電源ON操作|SysRS-01,SysRS-11a|電源ON操作は電源スイッチ短押し/長押しを想定。State06_TR-01と等価|
|State2_TR-02|State4_電源ON_DC-ON_異常 → 動作中|リリース|SysRS-16|State4_TR-06と等価|
|State2_TR-03|State1_電源ON_DC-OFF_正常 → 動作中|異常検知|SysRS-11a|State1_TR-06と等価|
|State2_TR-04|動作中 → 終了中|電源OFF操作|SysRS-11b,SysRS-15|電源OFF操作は電源スイッチ長押し/アバターコアからのシャットダウン操作を想定。終了中後の自動遷移によりState6_電源OFF_DC-OFF_異常となる想定|
|State2_TR-05|動作中 → State4_電源ON_DC-ON_異常|ドッキング[Warning1を除く]|SysRS-16|State4_TR-02と等価|
|State2_TR-06|動作中 → State3_電源ON_DC-ON_正常|ドッキング[Warning1]|SysRS-11b,SysRS-16|Warning1発生時はドッキングにより異常から復帰する想定。State3_TR-14と等価|
|State2_TR-07|動作中 → State1_電源ON_DC-OFF_正常|異常復帰操作[復帰操作可能異常]|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。Fault、Warning1は異常復帰操作は受け付けない想定。State1_TR-03と等価|

<div style="page-break-before:always"></div>

**State3_電源ON_DC-ON_正常**

![](.images/statemachine/State3_電源ON_DC-ON_正常.png)
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State3_TR-01|State7_電源OFF_DC-ON_正常 → 起動中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押しを想定（電源スイッチ長押しは無視される想定）。State7_TR-01と等価|
|State3_TR-02|State1_電源ON_DC-OFF_正常 → 動作中|ドッキング|SysRS-16|State1_TR-05と等価|
|State3_TR-03|State4_電源ON_DC-ON_異常 → 動作中|異常復帰操作|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。State4_TR-05と等価|
|State3_TR-04|動作中 → State1_電源ON_DC-OFF_正常|手動リリース|SysRS-03,SysRS-16|起動中は動作中に遷移後のリリース、終了中はState1_電源ON_DC-OFF_正常に遷移後のリリースと同じ振る舞いと想定し省略。State1_TR-02と等価。自動リリースについてはState3_TR-06参照|
|State3_TR-05|動作中 → State4_電源ON_DC-ON_異常|異常検知|SysRS-11a|起動中は動作中に遷移後の異常検知、終了中はState1_電源ON_DC-OFF_正常に遷移後の異常検知と同じ振る舞いと想定し省略。State4_TR-03と等価|
|State3_TR-06|自律走行中 → State1_電源ON_DC-OFF_正常（自律走行中）|DC-OFF検知|SysRS-03,SysRS-16|リリースの自動走行中にDC-OFFを検知した場合の振る舞い。State1_TR-04と等価|
|State3_TR-07|動作中 → 終了中|電源OFF操作|SysRS-15|電源OFF操作は電源スイッチ長押しを想定。終了中後はState7_電源OFF_DC-ON_正常に自動遷移を想定|
|State3_TR-08|リリース待ち無し → リリース待ち|自律走行指示|SysRS-03|自律走行指示はアバタークラウドからの充電ステーションからのリリース指示を想定|
|State3_TR-09|リリース待ち → リリース待ち無し|自律走行取消指示|SysRS-03|自律走行取消指示はアバタークラウドからのキャンセルコマンドを想定|
|State3_TR-10|リリース待ち → 自律走行中|安全確認操作|SysRS-03|安全確認操作は電源スイッチ短押しを想定|
|State3_TR-11|自律走行中 → リリース待ち無し|自律走行取消指示、安全確認取消操作|SysRS-03|自律走行取消指示ははアバタークラウドからのキャンセルコマンドを想定、安全確認取消操作は電源スイッチ短押しを想定|
|State3_TR-12|充電中 → 充電完了|満充電検知|SysRS-16|満充電検知はバッテリ充電100%を想定|
|State3_TR-13|充電完了 → 充電中|充電不足検知|SysRS-16|充電不足検知はバッテリ充電が閾値以下（具体値はTBD)を想定|
|State3_TR-14|State2_電源ON_DC-OFF_異常 → 動作中|ドッキング[Warning1]|SysRS-16|Warning1発生時はドッキングにより異常から復帰する想定。State2_TR-06と等価|

<div style="page-break-before:always"></div>

**State4_電源ON_DC-ON_異常**

![](.images/statemachine/State4_電源ON_DC-ON_異常.png)
...
|動作中|起動処理が完了し動作可能な状態|ここでは遠隔操作を受け付けない想定|
|終了中|終了処理を実施している状態|システム全体の終了を行う想定|
|DC-ON|充電ステーションに接続された状態|内部的には充電中、充電完了が存在するが外部観測できないため省略|
|Fault|Faultレベルの異常が発生している状態||
|復帰操作可能異常|復帰操作により正常に復帰できる状態|復帰操作は電源スイッチ短押しを想定。さらに内部状態として、Warning2、Cautionを包含する|
|Warning2|復帰操作可能異常状態内でWarning2レベルの異常が発生している状態||
|Caution|復帰操作可能異常状態内でCautionレベルの異常が発生している状態||
※:State4においてWarning1レベルの異常は発生しない想定

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State4_TR-01|State8_電源OFF_DC-ON_異常 → 起動中|電源ON操作|SysRS-01,SysRS-11a|電源ON操作は電源スイッチ短押しを想定（State8_電源OFF_DC-ON_異常での異常復帰操作との区別は？）。State8_TR-01と等価|
|State4_TR-02|State2_電源ON_DC-OFF_異常 → 動作中|ドッキング|SysRS-16|State2_TR-05と等価|
|State4_TR-03|State3_電源ON_DC-ON_正常 → 動作中|異常検知|SysRS-11a|State3_TR-05と等価|
|State4_TR-04|動作中 → 終了中|電源OFF操作|SysRS-11b,SysRS-15|電源OFF操作は電源スイッチ長押し/アバターコアからのシャットダウン操作を想定。終了中後の自動遷移によりState8_電源OFF_DC-ON_異常となる想定|
|State4_TR-05|動作中 → State3_電源ON_DC-ON_正常|異常復帰操作[復帰操作可能異常]|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。Faultは異常復帰操作は受け付けない想定。State3_TR-03と等価|
|State4_TR-06|動作中 → State2_電源ON_DC-OFF_異常|リリース|SysRS-16|State2_TR-02と等価|

<div style="page-break-before:always"></div>

**State5_電源OFF_DC-OFF_正常**

![](.images/statemachine/State5_電源OFF_DC-OFF_正常.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|State5_電源OFF_DC-OFF_正常|電源OFFで充電ステーション未接続で正常な状態|State6_電源OFF_DC-OFF_異常と外部観測で区別できない。電源ON操作やドッキングにより遷移した先の違いにより事後的に判別される|

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State5_TR-01|State5_電源OFF_DC-OFF_正常 → State1_電源ON_DC-OFF_正常|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定。State1_TR-01と等価|
|State5_TR-02|State5_電源OFF_DC-OFF_正常 → State7_電源OFF_DC-ON_正常|ドッキング|SysRS-16||
|State5_TR-03|State5_電源OFF_DC-OFF_正常 → State6_電源OFF_DC-OFF_異常|異常検知|SysRS-11a|外部観測はできない|

<div style="page-break-before:always"></div>

**State6_電源OFF_DC-OFF_正常**

![](.images/statemachine/State6_電源OFF_DC-OFF_異常.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|State6_電源OFF_DC-OFF_異常|電源OFFで充電ステーション未接続で異常が発生している状態|State5_電源OFF_DC-OFF_正常と外部観測で区別できない。電源ON操作やドッキングにより遷移した先の違いにより事後的に判別される|

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State6_TR-01|State6_電源OFF_DC-OFF_異常 → State2_電源ON_DC-OFF_異常|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定。State2_TR-01と等価|
|State6_TR-02|State6_電源OFF_DC-OFF_異常 → State8_電源OFF_DC-ON_異常|ドッキング|SysRS-16||
|State6_TR-03|State6_電源OFF_DC-OFF_異常 → State5_電源OFF_DC-OFF_正常|異常復帰操作||ありえない遷移|

<div style="page-break-before:always"></div>

**State7_電源OFF_DC-ON_正常**

![](.images/statemachine/State7_電源OFF_DC-ON_正常.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|電源OFF|電源OFFで充電ステーション接続され正常な状態|異常検知し状態表示可能なように部分的に電源ONされる想定|
|充電中|バッテリが充電中である状態||
|充電完了|バッテリの充電が完了している状態||
|正常|異常が無いことを示す状態||
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State7_TR-01|State7_電源OFF_DC-ON_正常 → State3_電源ON_DC-ON_正常|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押しを想定（電源スイッチ長押しは何もしない想定）。State3_TR-01と等価|
|State7_TR-02|State7_電源OFF_DC-ON_正常 → State5_電源OFF_DC-ON_正常|リリース|SysRS-15,SysRS-16|リリースは手動リリースのみ|
|State7_TR-03|State7_電源OFF_DC-ON_正常 → State8_電源OFF_DC-ON_異常|異常検知|SysRS-11a||
|State7_TR-04|充電中 → 充電完了|満充電検知|SysRS-16|満充電検知はバッテリ充電100%を想定|
|State7_TR-05|充電完了 → 充電中|充電不足検知|SysRS-16|充電不足検知はバッテリ充電が閾値以下（具体値はTBD)を想定|

<div style="page-break-before:always"></div>

**State8_電源OFF_DC-ON_異常**

![](.images/statemachine/State8_電源OFF_DC-ON_異常.png)

上記の状態遷移（ステートマシン図）より、要求に影響を与える内部状態および遷移を以下に示す

|状態|定義|備考|
|:---|:---|:---|
|電源OFF|電源OFFで充電ステーション接続され異常のある状態|異常検知し状態表示可能なように部分的に電源ONされる想定|
|DC-ON|充電ステーションに接続された状態|内部的には充電中、充電完了が存在するが外部観測できないため省略|
|Fault|Faultレベルの異常が発生している状態||
|復帰操作可能異常|復帰操作により正常に復帰できる状態|復帰操作は電源スイッチ短押しを想定。さらに内部状態として、Warning2、Cautionを包含する|
|Warning2|復帰操作可能異常状態内でWarning2レベルの異常が発生している状態||
|Caution|復帰操作可能異常状態内でCautionレベルの異常が発生している状態||
※:State8においてWarning1レベルの異常は発生しない想定

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|State8_TR-01|State8_電源OFF_DC-ON_異常 → State4_電源ON_DC-ON_異常|電源OFF操作|SysRS-01,SysRS-11a|電源ON操作は電源スイッチ短押しを想定（State8_TR-03との区別は？）。State4_TR-01と等価|
|State8_TR-02|State8_電源OFF_DC-ON_異常 → State6_電源OFF_DC-OFF_異常|リリース|SysRS15,SysRS-16||
|State8_TR-03|State8_電源OFF_DC-ON_異常 → State7_電源OFF_DC-OFF_正常|異常復帰操作|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定（State4_TR-01との区別は？）|
...
- 備考
    ...
    - State5_TR-01、State6_TR-01より、
      - 電源OFFで充電ステーションに未接続である状態から充電ステーションに接続された場合は、ロボットの一部のみ起動する（充電中/充電完了/Fault/Warning2/Cautionの状態表示する（起動中の状態表示しない））
        - ロボットの一部はMiimo Main ECUと表示LEDの起動を想定
    - State7_TR-01、State8_TR-01より
      - 電源OFFで充電ステーションに接続された状態で、電源ON操作をされた場合は、ロボット全体を起動する（起動中の状態表示する）
        - 上記電源ON操作は電源スイッチ短押し操作である（電源スイッチ長押し操作ではない）想定
        - 電源OFFで充電ステーションに接続された異常が発生している状態での電源スイッチ操作について要確認
...
- 備考
	- 本ユースケース（UC）の実行中に、以下の各UCも実行される。詳細については各UCの記述を参照のこと
        ...
        - バッテリ残量低下を検知し停止する
        - 異常を検知し停止する
    - State1_TR-10、State1_TR-11、State1_TR-12、State1_TR-13より
      - State1_電源ON_DC-OFF_正常状態において、遠隔操作待ち状態では走行操作指示（走行操作コマンド）または自律走行開始指示（帰還コマンド）を受け付ける
        - 走行操作指示を受付後、走行操作中状態となる
        - 自律走行開始指示を受付後、帰還動作を開始する（自律走行中状態に遷移する）とする
      - 走行操作中状態では、速度0検知まで走行操作指示のみ受け付ける
      - 自律走行中状態では、キャンセル指示のみ受け付ける
        - キャンセル指示を受付後、遠隔操作待ち状態となる
        - 自律走行がリリース動作であった場合、リリース動作完了後、遠隔操作待ち状態に自動遷移する
    - State3_TR-08、State3_TR-09、State3_TR-10、State3_TR-11より
      - State3_電源ON_DC-ON_正常状態において、リリース待ち無し状態では自律走行開始指示（リリースコマンド）のみを受け付ける、受付後、リリース待ち状態（状態表示はリリース待ち）になる
      - リリース待ち状態では安全確認操作（電源スイッチ短押し）か自動走行取消指示（キャンセルコマンド）を受け付ける。
        - 安全確認操作を受付後、リリース動作を開始する（自律走行中状態に遷移する)とする
        - 自律走行取消指示を受付後、リリース待ち無し状態に戻る（状態表示は、充電中か充電完了）とする
      - 自律走行中状態（DC-OFF検知前）では自動走行取消指示または安全確認取消操作（電源スイッチ短押し）を受け付け、リリース待ち無し状態に戻る（状態表示は、充電中か充電完了）とする
      - 自律走行中状態でDC-OFF検知後は、State1_電源ON_DC-OFF_正常状態の自律走行中状態となる
...
- 備考
    - State1_TR-09より
      - DC-OFF状態（充電ステーションから離れている電源ON状態）場合、一時停止操作（状態遷移分析では遠隔操作禁止操作と表記）により一時停止中状態（状態遷移分析では遠隔操作不可状態と表記）になるものとする。
...
- 備考
	- State1_TR-08により
    	- 一時停止状態での一時停止解除操作（状態遷移分析では遠隔操作可能指示）は遠隔操作可能状態内の遠隔操作待ちとなるものとする
...
- 備考
    - State1_TR-06、State2_TR-01、State3_TR-05、State4_TR-01、State7_TR-03より
      - 充電ステーション接続中はWarning1があり得ない想定
...
- 備考
	- テレプレゼンスロボットを再起動してもFault状態が解消されない場合はメンテナンスが必要
	- State1_TR-03、State2_TR-04、State2_TR-06、State3_TR-03、State4_TR-04、State8_TR-03より
    	- Fault、Warning1は電源スイッチ短押しによる復帰操作を受け付けない
    	- Warning1はドッキングすることにより正常状態へ遷移する
    	- Warning2、Caution状態では電源スイッチ短押しによる復帰操作を受け付ける
    	- Faultは電源OFF操作（電源スイッチ長押し/シャットダウン操作）を行い、電源OFF状態になった後に、電源ON操作（電源スイッチ短押しおよび充電ステーション未接続であれば電源スイッチ長押しも可）を行うことで正常状態への復帰を想定
    	- State8_電源OFF_DC-ON_異常状態からの復帰操作の扱いについては要確認
...
- 事後条件
    ...
    - 電源OFFとなること
- 備考
    - State1_TR-07、State2_TR-04、State3_TR-07、State4_TR-04より
      - 電源スイッチ長押し/シャットダウン操作により終了状態を経て電源OFF状態に遷移する想定
    - State7_TR-02、State8_TR-02より
      - State7_電源OFF_DC-ON_正常状態およびState8_電源OFF_DC-ON_異常状態で手動リリースされた場合、ロボット全体を終了する（状態表示は終了中表示しない）
        - Miimo Main ECUの終了処理を想定
      - State8_電源OFF_DC-ON_異常状態での電源スイッチ短押し操作の挙動については要確認
...
- 事後条件
    - 充電を停止する
- 備考
    - State1_TR-02、State1_TR-05、State2_TR-02、State2_TR-05、State2_TR-06、State5_TR-02、State6_TR-02、State7_TR-02、State8_TR-02より
      - 充電ステーション接続中は、異常有無、リリース待ち有無に関わらず、内部的には充電中か充電完了が制御されている
      - State4_電源ON_DC-ON_異常状態またはState8_電源OFF_DC-ON_異常状態であれば、異常状態を優先して表示する
      - State3_電源ON_DC-ON_正常状態の場合、リリース待ち状態を優先して表示する
      - State7_電源OFF_DC-ON_正常状態の場合、充電中/充電完了状態を表示する
      - Warning1異常発生時に充電ステーション接続された場合は、異常から復帰し正常状態へ遷移する
...
|走行部は充電ステーションから取り出す||
|走行部は充電ステーション未接続中に充電を停止する|DC-OFF時|
