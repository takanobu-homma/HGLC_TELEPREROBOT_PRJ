...
|REQ-022|テレプレゼンスロボットを非常停止ボタンにより停止する|UC-011, UC-012||
|REQ-023|テレプレゼンスロボットを非常停止を通知する|UC-011, UC-012||
|REQ-024|テレプレゼンスロボットの非常停止状態を解除する|UC-011, UC-012||
|REQ-025|テレプレゼンスロボットを非常停止解除を通知する|UC-011, UC-012||
...
この一覧について、テレプレゼンスロボット本体を頭部、胴体部、走行部の3ブロックに分割し、各ブロックレベルで詳細化を行う。
...
|要求|備考|
|:---|:---|
|頭部は操作コマンドを受け取りCAN信号に変換する||
|走行部はCAN信号から動作命令を取り出し、ベース動作命令を走行部に首動作命令を胴体部に分ける||
|胴体部は首動作命令（初期化、水平動作、垂直動作）をモータ駆動信号に変換する||
|胴体部はモータ駆動信号（初期化）により首位置をリセットする||
|胴体部はモータ駆動信号（水平動作）により首を指定された角度で水平に動かす||
|胴体部はモータ駆動信号（垂直動作）により首を指定された角度で垂直に動かす||
|胴体部はYES動作命令をモータ駆動信号に変換し首を上下に2回動かす||
|胴体部はNO動作命令をモータ駆動信号に変換し首を左右に2回動かす||
|走行部はベース動作命令（走行、旋回）をモータ駆動信号に変換する||
|走行部はモータ駆動信号（走行）により前後に走行する||
|走行部はモータ駆動信号（旋回）により左右に旋回する||
|走行部は帰還指示命令により、走行動作、旋回動作を組み合わせ充電ステーションにドッキングする||
