...
|頭部はステーション検出値によりステーション近傍を判断する||
|頭部はステーション近傍に近づいたら、それをアバタークラウドに通知する||
...
ロボット内で扱われる情報としては以下のような概念モデルを想定（末端のクラスはL2以降で精査）

![](.images/conceptual/robot_info.png)
...
- 例外フロー
    - todo
- 備考
	- アバタークラウドが知りたい（≒アバターアプリ利用ユーザ）が知りたい状態とは？
        - 要求分析結果からすると、
          - 各センサーの状態
            - センサーの洗い出しはL2レベルで良いか？
          - 起動処理の完了
          ...
          - バッテリ残量低下による停止
            - 通常の停止とは区別した通知が必要か？
          - 一時停止
            - 一時停止操作の実行により発火する通知と考えて良いか？
          - 一時停止解除
            - 一時停止解除操作の実行により発火する通知と考えて良いか？
          - 充電状況
            - 充電量の閾値をまたぐことで発火する通知と考えて良いか？
            - 「状態を表示する」の方は「充電不足」か「充電が足りている」だが、通知して欲しい粒度は、10%刻みや充電余裕、充電不足間近、充電不足のように異なることは無いか？
          - 明るさの不足
            - 明るさの閾値をまたぐことで発火する通知と考えて良いか？
        - 他には無いか？
...
|要求|備考|
|:---|:---|
|頭部はロボットの状態を判断しアバタークラウドに送信する|・送信する情報はL2以降で精査|
