...
|TR0-03|動作中の開始 → 一時停止|開始状態からの自動遷移[DC-OFF]|SysRS-01,SysRS-06|開始状態はTR0-14/TR0-17による遷移を想定|
|TR0-04|動作中の開始 → ステーション待機中|開始状態からの自動遷移[DC-ON]|SysRS-01|開始状態はTR0-14/TR0-17による遷移を想定|
...
|TR0-14|異常 → 動作中|異常復帰操作|SysRS-11b|電源スイッチ短押しを想定。動作中遷移後はTR0-03/TR0-04で分岐する想定|
...
|TR0-17|起動中 → 動作中|起動中の完了による自動遷移|SysRS-01|動作中遷移後はTR0-03/TR0-04で分岐する想定|
...
- 備考
	- テレプレゼンスロボットを再起動してもFault状態が解消されない場合はメンテナンスが必要
    - TR0-02,TR0-03,TR0-04,TR0-14,TR3-05より
