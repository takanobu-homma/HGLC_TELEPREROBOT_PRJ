...
|---:|:---|:---|:---|:---|
|TR1-01|ST05_電源OFF → 起動中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定|
|TR1-02|ST03_充電中 → 動作中（遠隔操作不可）|手動リリース|SysRS-06,SysRS-16|自動リリースをトリガとする遷移はTR1-04参照|
|TR1-03|ST02_異常 → 動作中（遠隔操作不可）|異常復帰操作|SysRS-06,SysRS-11b|異常復帰操作は電源スイッチ短押しを想定|
|TR1-04|ST03_充電中（自律走行中） → 動作中（遠隔操作可能（自律走行中））|DC-OFF検知|SysRS-03,SysRS-16|リリースの自動走行中にDC-OFFを検知した場合の振る舞い|
|TR1-05|動作中 → ST03_充電中（動作中）|ドッキング|SysRS-03,SysRS-16|起動中は動作中に遷移後のドッキング、終了中はST05_電源OFFに遷移後のドッキングと同じ振る舞いと想定し省略|
|TR1-06|動作中 → ST02_異常（動作中）|異常検知|SysRS-11a|起動中は動作中に遷移後の異常検知、終了中はST05_電源OFFに遷移後の異常検知と同じ振る舞いと想定し省略|
|TR1-07|動作中/リプロ/ST09_テスト → 終了中|電源OFF操作|SysRS-15|終了後はST05_電源OFFに自動遷移を想定|
|TR1-08|遠隔操作不可 → 遠隔操作可能|遠隔操作可能指示|SysRS-03,SysRS-07|遠隔操作可能指示は電源スイッチ短押しを想定|
|TR1-09|遠隔操作可能 → 遠隔操作不可|遠隔操作禁止指示|SysRS-03,SysRS-06|遠隔操作禁止指示は電源スイッチ短押しを想定|
|TR1-10|遠隔操作待ち/走行操作中 → 走行操作中|走行操作指示|SysRS-03|走行操作指示はアバタークラウドからの前進/後進/旋回指示を想定|
|TR1-11|走行操作中 → 遠隔操作待ち|速度0検知|SysRS-03|走行操作指示がされず、速度0となった場合に遠隔操作待ちとなることを想定|
|TR1-12|遠隔操作待ち → 自律走行中|自律走行指示|SysRS-03|自律走行指示はアバタークラウドからの充電ステーションへの帰還指示を想定|
|TR1-13|自律走行中 → 遠隔操作待ち|キャンセル指示/自動リリースの完了|SysRS-03|※1|
|TR1-14|遠隔操作不可 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|
|TR1-15|遠隔操作不可 → リプロ|リプロモード遷移指示|SysRS-22|リプロモード遷移指示は書き換え用PCから受信する想定|

※1:キャンセル指示はアバタークラウドからの充電ステーションへの帰還指示キャンセルおよび充電ステーションからのリリース指示キャンセルを想定。リリース指示による自律走行中の完了に伴う自動遷移によっても遠隔操作待ちとなる想定。遠隔操作禁止指示（電源スイッチ短押しを想定）はTR1-09となる想定  
...
|:---|:---|:---|
|起動中|起動処理を実施している状態|システム全体の起動を行う想定|
|動作中|起動処理が完了し動作可能な状態|ここでは遠隔操作を受け付けない想定。システム全体が起動（異常により頭部がOFFである場合も含む）している想定|
|終了中|終了処理を実施している状態|システム全体の終了を行う想定|
...
|---:|:---|:---|:---|:---|
|TR2-01|ST06_電源OFF異常 → 起動中|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|電源ON操作は電源スイッチ短押し/長押しを想定|
|TR2-02|ST04_充電中異常 → 動作中|リリース|SysRS-16||
|TR2-03|ST01_正常運転 → 動作中|異常検知|SysRS-11a||
|TR2-04|動作中/ST09_テスト → 終了中|電源OFF操作|SysRS-11b,SysRS-15|※1|
|TR2-05|動作中 → ST04_充電中異常|ドッキング[Warning1を除く]|SysRS-16||
|TR2-06|動作中 → ST03_充電中|ドッキング[Warning1]|SysRS-11b,SysRS-16|Warning1発生時はドッキングにより異常から復帰する想定|
|TR2-07|動作中 → ST01_正常運転|異常復帰操作[復帰操作可能異常]|SysRS-06,SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。Fault、Warning1は異常復帰操作は受け付けない想定|
|TR2-08|動作中 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR3-01|ST07_通電中 → 起動中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押しを想定（電源スイッチ長押しは無視される想定）|
|TR3-02|ST01_正常運転 → 動作中|ドッキング|SysRS-03,SysRS-16||
|TR3-03|ST04_充電中異常 → 動作中|異常復帰操作|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定|
|TR3-04|動作中 → ST01_正常運転|手動リリース|SysRS-06,SysRS-16|※1|
|TR3-05|動作中 → ST04_充電中異常|異常検知|SysRS-11a|※2|
|TR3-06|自律走行中 → ST01_正常運転（自律走行中）|DC-OFF検知|SysRS-03,SysRS-16|リリースの自動走行中にDC-OFFを検知した場合の振る舞い|
|TR3-07|動作中/ST09_テスト → 終了中|電源OFF操作|SysRS-15|電源OFF操作は電源スイッチ長押しを想定。終了中後はST07_通電中に自動遷移を想定|
|TR3-08|リリース待ち無し → リリース待ち|自律走行指示|SysRS-03|自律走行指示はアバタークラウドからの充電ステーションからのリリース指示を想定|
|TR3-09|リリース待ち → リリース待ち無し|自律走行取消指示|SysRS-03|自律走行取消指示はアバタークラウドからのキャンセルコマンドを想定|
|TR3-10|リリース待ち → 自律走行中|安全確認操作|SysRS-03|安全確認操作は電源スイッチ短押しを想定|
|TR3-11|自律走行中 → リリース待ち無し|自律走行取消指示、安全確認取消操作|SysRS-03|※3|
|TR3-12|充電中 → 充電完了|満充電検知|SysRS-16|満充電検知はバッテリ充電100%を想定|
|TR3-13|充電完了 → 充電中|充電不足検知|SysRS-16|充電不足検知はバッテリ充電が閾値以下（具体値はTBD)を想定|
|TR3-14|ST02_異常 → 動作中|ドッキング[Warning1]|SysRS-11b,SysRS-16|Warning1発生時はドッキングにより異常から復帰する想定|
|TR3-15|リリース待ち無し → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|

※1:起動中は動作中に遷移後のリリース、終了中はST01_正常運転に遷移後のリリースと同じ振る舞いと想定し省略。自動リリースについてはTR3-06参照  
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR4-01|ST08_通電中異常 → 起動中|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|※1|
|TR4-02|ST02_異常 → 動作中|ドッキング|SysRS-16||
|TR4-03|ST03_充電中 → 動作中|異常検知|SysRS-11a||
|TR4-04|動作中/ST09_テスト → 終了中|電源OFF操作|SysRS-11b,SysRS-15|※2|
|TR4-05|動作中 → ST03_充電中|異常復帰操作[復帰操作可能異常]|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定。Faultは異常復帰操作は受け付けない想定|
|TR4-06|動作中 → ST02_異常|リリース|SysRS-16||
|TR4-07|動作中 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|
...
ST05_電源OFFは内部状態を持たない（ST06_電源OFF異常と外部観測で区別できない。電源ON操作やドッキングにより遷移した先の違いにより事後的に判別される）

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR5-01|ST05_電源OFF → ST01_正常運転|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押し/長押しを想定|
|TR5-02|ST05_電源OFF → ST07_通電中|ドッキング|SysRS-16||
|TR5-03|ST05_電源OFF → ST06_電源OFF異常|異常検知||外部観測不能な遷移|
...
ST06_電源OFF異常は内部状態を持たない（ST05_電源OFFと外部観測で区別できない。電源ON操作やドッキングにより遷移した先の違いにより事後的に判別される）

本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR6-01|ST06_電源OFF異常 → ST02_異常|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|電源ON操作は電源スイッチ短押し/長押しを想定※1|
|TR6-02|ST06_電源OFF異常 → ST08_通電中異常|ドッキング|SysRS-16||
|TR6-03|ST06_電源OFF異常 → ST05_電源OFF|異常復帰操作||外部観測不能な遷移|
...
※:ST07_通電中では異常検知と状態表示が可能なように部分的に電源ONされる想定


本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR7-01|ST07_通電中 → ST03_充電中|電源ON操作|SysRS-01|電源ON操作は電源スイッチ短押しを想定（電源スイッチ長押しは何もしない想定）|
|TR7-02|ST07_通電中 → ST05_電源OFF|リリース|SysRS-15,SysRS-16|リリースは手動リリースのみ|
|TR7-03|ST07_通電中 → ST08_通電中異常|異常検知|SysRS-11a||
|TR7-04|充電中 → 充電完了|満充電検知|SysRS-16|満充電検知はバッテリ充電100%を想定|
|TR7-05|充電完了 → 充電中|充電不足検知|SysRS-16|充電不足検知はバッテリ充電が閾値以下（具体値はTBD)を想定|
...
※※:ST08_通電中異常では異常検知と状態表示が可能なように部分的に電源ONされる想定


本状態に関する状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR8-01|ST08_通電中異常 → ST03_充電中|電源ON操作|SysRS-01,SysRS-11a,SysRS-11b|※1|
|TR8-02|ST08_通電中異常 → ST06_電源OFF異常|リリース|SysRS-15,SysRS-16|※2|
|TR8-03|ST08_通電中異常 → ST03_充電中|異常復帰操作|SysRS-11b|※1|
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR9-01|ST01_正常運転/ST02_異常/ST03_充電中/ST04_充電中異常 → ST09_テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから受信する想定|
|TR9-02|ST09_テスト → (ST01_正常運転/ST02_異常/ST03_充電中/ST04_充電中異常)終了中|電源OFF操作|SysRS-15|テスト終了時は、終了中を経て電源OFF状態となる想定|
...
- 備考
    ...
    - TR5-01(TR1-01)、TR6-01(TR2-01)より、
    ...
    - TR7-01(TR3-01)、TR8-01(TR4-01)より
...
- 備考
    ...
    - TR1-05(TR3-02)、TR1-10、TR1-11、TR1-12、TR1-13より
    ...
    - TR1-04(TR3-06)、TR3-08、TR3-09、TR3-10、TR3-11より
      - ST03_充電中>動作中において
...
- 備考
    ...
    - TR1-02(TR3-04)、TR1-03(TR2-07)より
...
- 備考
	- テレプレゼンスロボットを再起動してもFault状態が解消されない場合はメンテナンスが必要
	- TR1-03(TR2-07)、TR3-03(TR4-05)より
...
- 事後条件
    - 充電を停止する
- 備考
    - TR1-05(TR3-02)、TR2-05(TR4-02)、TR2-06(TR3-02)、TR3-12、TR3-13、TR3-14、TR5-02、TR6-02、TR7-04、TR7-05より
    ...
    - TR1-02(TR3-04)、TR1-04(TR3-06)、TR2-02(TR4-06)、TR7-02、TR8-02より
...
- 備考
	- TR9-01(TR1-14/TR2-08/TR3-15/TR4-07)より
    	- テストモード遷移指示を診断PCから受信することによりテストモードとなる想定
  	- TR9-02(TR1-07/TR2-04/TR3-07/TR4-04)より
