...
|TR0-05|動作中/異常 → テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから送信される想定※1|
|TR0-06|動作中 → リプロ|リプロモード遷移指示|SysRS-22|リプロモード遷移指示は書き換え用PCから送信される想定※2|
|TR0-07|起動中 → 動作中|起動中の完了による自動遷移|SysRS-01|動作中遷移後はTR2-01/TR2-02で分岐する想定|

※1:動作中からは内部状態の一時停止およびステーション待機中内のリリース待ち無しから遷移する想定。一時停止については、「SM-02:動作中」参照。リリース待ち無しについては、「SM-02-1:ステーション待機中」参照  
※2:動作中の内部状態の一時停止から遷移する想定。一時停止については、「SM-02:動作中」参照
