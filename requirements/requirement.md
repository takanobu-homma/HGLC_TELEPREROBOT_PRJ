...
|テスト|テストモードで起動している状態|内部状態は「SM-05:テスト」を参照※1|
|リプロ|リプロモードで起動している状態|※2|

※:DC-ON検知時はサブシステム（バッテリ）の状態として「SM-04:バッテリ」の内部状態が制御されている想定  
※1:診断PCからのテストモード遷移指示により遷移し、終了時は診断PCからのテストモード解除指示によりテストモード遷移前の状態に戻る（または電源OFF操作により電源OFFとなる）想定  
※2:書き換え用PCからのリプロモード遷移指示により遷移し、終了時は電源OFF操作（書き換え用PCからのシャットダウン指示も電源OFF操作となる）により電源OFFとなる想定  
...
|※2|EP2-1 → テスト|なし(自動遷移)|SysRS-19<br/>SysRS-20<br/>SysRS-21|遷移元（動作中）内部でのテストモード遷移指示により遷移する想定|
|※2|EP2-2 → リプロ|なし(自動遷移)|SysRS-22|遷移元（動作中）内部でのリプロモード遷移指示により遷移する想定|
|※3|EP3-1 → 動作中|なし(自動遷移)|SysRS-11b|遷移元（異常）内部での異常復帰操作により遷移する想定|
|※4|EP5-1 → 動作中|なし(自動遷移)|SysRS-19<br/>SysRS-20<br/>SysRS-21|遷移元（テスト）内部でのテストモード解除指示により遷移する想定|
|※4|EP5-2 → 異常|なし(自動遷移)|SysRS-11a<br/>SysRS-19<br/>SysRS-20<br/>SysRS-21|遷移元（テスト）内部でのテストモード解除指示により遷移する想定|


※1:リプロの場合、書き換え用PCからのシャットダウン指示も電源OFF操作となる    
※2:遷移元（動作中）の詳細は「SM-02:動作中」参照  
※3:遷移元（異常）の詳細は「SM-03:異常」参照  
※4:遷移元（テスト）の詳細は「SM-05:テスト」参照
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR1-01|開始 → 状態表示|電源OFF操作[DC-ON]|SysRS-15|状態表示/異常検知できる範囲を残してロボットを終了する想定（終了中の状態表示する）|
|TR1-02|開始 → 状態非表示|電源OFF操作[ELSE]|SysRS-15|ロボット全体を終了する想定（終了中の状態表示する）|
...
|※3※5|Sub-EP1 → EP2-1|なし(自動遷移)|SysRS-19<br/>SysRS-20<br/>SysRS-21|遷移元（ステーション待機中）内部でのテストモード遷移指示により遷移する想定|
|※5|Sub-EP2 → リリース移動中|なし(自動遷移)|SysRS-03|遷移元（ステーション待機中）内部での自動リリースによるDC-OFF検知により遷移する想定|
|※5|Sub-EP3 → 一時停止|なし(自動遷移)|SysRS-06|遷移元（ステーション待機中）内部での手動リリースによるDC-OFF検知により遷移する想定|
