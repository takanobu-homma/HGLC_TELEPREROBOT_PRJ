...
|SysRS-10|バッテリ残量低下を検知し停止する|テレプレゼンスロボットのバッテリ残量低下を検知し停止する|・アバタークラウド|REQ-020|
|SysRS-11|異常を検知し停止する|テレプレゼンスロボット内部の異常を検知し停止する|・アバタークラウド|REQ-026|
...
|SysRS-21|動作確認を実施する|テレプレゼンスロボットは診断PCと接続・通信し、頭部・胴体部・走行部のモーター等動作確認等を行う|・サービスエンジニア|REQ-036,<br/>REQ-037,<br/>REQ-040,<br/>REQ-041|
...
尚、この一覧に記載されていないREQ-033, REQ-046, REQ-047は機能的な振る舞いとは異なる要求となるため、この一覧とは別に扱う(SysRS-18、SysRS-23はこれら要求を含むユースケースで使用されている)
...
|状態|定義|備考|
|:---|:---|:---|
|電源OFF|テレプレゼンスロボットに電源が入っていない状態（充電端子に接続されていない）||
|異常無し|テレプレゼンスロボットに電源が入っている状態（異常が検知されていない）|通電中、頭部電源ONを包含|
|通電中|テレプレゼンスロボットが充電端子に接続されており頭部電源がOFFである状態|充電中、充電完了を包含|
|充電中|DC-ON状態または通電中状態の中でバッテリーを充電している状態||
|充電停止|DC-ON状態または通電中状態の中でバッテリーの充電が完了している状態||
|頭部電源ON|テレプレゼンスロボットの頭部電源がONである状態|起動中、正常動作中、終了中を包含|
|起動中|頭部電源ON状態の中で起動処理を行っている状態||
|正常動作中|頭部電源ON状態の中で起動処理が正常終了した状態|DC-ON、DC-OFFを包含|
|DC-ON|正常動作中状態の中で充電端子に接続されている状態|リリース待ち無し、リリース待ち1、リリース待ち2、リリース中(DC-ON)、充電中、充電停止を包含|
|リリース待ち無し|DC-ON状態の中でリリース動作を受け付けていない状態||
|リリース待ち1|DC-ON状態の中でリリースコマンドを受け付けた状態|リリースコマンドは頭部からの指示を想定（リリース開始指示後、リリース動作を開始する）|
|リリース待ち2|DC-ON状態の中でリリース開始指示を受け付けた状態|リリース開始指示は電源スイッチ短押しの想定（リリースコマンド受付後、リリース動作を開始する）<br/>リリース開始指示は一定期間内にリリースコマンドが無い場合、タイムアウトする想定|
|リリース中(DC-ON)|DC-ON状態の中でリリース動作を行っている状態|DC-OFF検知により、リリース中(DC-OFF)に遷移する想定|
|DC-OFF|正常動作中状態の中で充電端子に接続されていない状態|一時停止中、遠隔操作中、ドッキング中、リリース中(DC-OFF)を包含|
|一時停止中|DC-OFF状態の中で一時停止している状態|外部操作を受け付けない状態を想定|
|遠隔操作中|DC-OFF状態の中で外部から遠隔操作が行える状態|頭部からCAN通信で走行部を制御できる状態を想定|
|ドッキング中|DC-OFF状態の中でドッキング指示によりドッキング動作を行っている状態|ドッキング動作終了後はリリース待ち無しに自動遷移する想定|
|リリース中(DC-OFF)|DC-OFF状態の中でリリース動作を行っている状態|リリース動作終了後は遠隔操作中に遷移する想定|
|終了中|頭部電源ON状態の中で終了処理を行っている状態||
|異常停止中|異常を検出している状態|Fault、Warning、Cautionを包含|
|Fault|動作中状態の中でデバイス自体が破損している、異常を検出し、走行停止している状態|再起動による復帰を必要とするエラーを想定|
|Warning|動作中状態の中で異常検出後、走行動作を停止している状態|Warning1、Warning2を包含|
|Caution|動作中状態の中で保護機能が動作し、走行動作を停止している状態|過負荷作業警告などのエラーを想定|
|Warning1|動作中状態の中でバッテリ残量低下により走行ができない状態|バッテリ残量低下を想定|
|Warning2|動作中状態の中で異常検知し走行ができない状態|走行モータ回転異常などを想定|
|リプロ|リプロモードで起動している状態|（TODO）|
|テスト|テストモードで起動している状態|（TODO）|

### 状態遷移図

テレプレゼンスロボット全体の状態遷移図を以下に記載する

![](.images/statemachine/st01.png)
...
上記の状態遷移（ステートマシン図）より、ロボット本体の各ユースケースの要求詳細化の際に考慮すべき状態遷移について下記の表に記述する。

尚、状態遷移より導出される要求については影響を受ける要求で個別に抽出を行う

<!-- TODO:状態遷移図に番号をつける-->

|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|1|電源OFF → 通電中|手動ドッキング|SysRS-01<br/>SysRS-16|SM-01、SM-02参照|
|2|通電中 → 電源OFF|手動リリース|SysRS-15<br/>SysRS-16|SM-01、SM-02参照|
|3|電源OFF → 頭部電源ON|電源ON操作|SysRS-01|SM-01、SM-03参照<br/>電源ON操作は電源スイッチ短押し、スイッチ長押し操作を想定|
|4|頭部電源ON → 電源OFF|電源OFF操作[充電端子未接続]|SysRS-15|SM-01、SM-03参照<br/>電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|5|頭部電源ON → 通電中|電源OFF操作[充電端子接続]|SysRS-15<br/>SysRS-16|SM-02参照<br/>電源OFF操作はシャットダウン操作、電源スイッチ長押し操作を想定|
|6|通電中 → 頭部電源ON|電源ON操作|SysRS-01<br/>SysRS-16|SM-02参照<br/>電源ON操作は電源スイッチ短押し操作を想定|
|7|正常動作中 → Fault|Fault検知|SysRS-11|SM-04、SM-08参照|
|8|正常動作中 → Warning1|バッテリ残量低下検知|SysRS-10|SM-04、SM-08参照|
|9|正常動作中 → Warning2|その他Warning検知|SysRS-11|SM-04、SM-08参照<br/>Warning1が優先|
|10|正常動作中 → Caution|Caution検知|SysRS-11|SM-04、SM-08参照|
|11|Warning1 → 正常動作中|異常復帰操作|SysRS-10|SM-04、SM-09参照<br/>充電ステーションへの接続でWarning1は解除される（Warning2はされない）|
|12|Warning2 → 正常動作中|異常復帰操作|SysRS-11|SM-04、SM-09参照<br/>スイッチ短押しで復帰|
|13|Caution → 正常動作中|異常復帰操作|SysRS-11|SM-04、SM-08参照<br/>スイッチ短押しで復帰|
|14|異常停止中 → 電源OFF|電源OFF操作[充電端子未接続]|SysRS-11<br/>SysRS-15|SM-01、SM-08参照<br/>充電端子未接続時はスイッチ長押し、シャットダウン操作で電源OFFになる|
|15|異常停止中 → 通電中|電源OFF操作[充電端子接続]|SysRS-11<br/>SysRS-15<br/>SysRS-16|SM-01、SM-08参照<br/>充電端子接続時は頭部電源ONであれば、スイッチ長押し、シャットダウン操作で通電中になる。頭部電源OFFであれば、スイッチ長押しは無視される|
|16|DC-OFF → DC-ON|充電端子接続|SysRS-16|SM-05参照|
|17|DC-ON → DC-OFF|充電端子接続解除|SysRS-16|SM-05参照|
|18|リリース待ち無し → リリース待ち1|リリースコマンド受付|SysRS-03<br/>SysRS-16|SM-06参照|
|19|リリース待ち無し → リリース待ち2|リリース開始指示受付|SysRS-03<br/>SysRS-16|SM-06参照<br/>リリース開始指示は電源スイッチ短押しの想定|
|20|リリース待ち1 → リリース中(DC-ON)|リリース開始指示受付|SysRS-03<br/>SysRS-16|SM-06参照<br/>リリース開始指示は電源スイッチ短押しの想定|
|21|リリース待ち1 → リリース待ち無し|キャンセルコマンド受付|SysRS-03<br/>SysRS-16|SM-06参照<br/>タイムアウトはしない想定|
|22|リリース待ち2 → リリース中(DC-ON)|リリースコマンド受付|SysRS-03<br/>SysRS-16|SM-06参照|
|23|リリース待ち2 → リリース待ち無し|リリース開始解除指示受付 or タイムアウト|SysRS-03<br/>SysRS-16|SM-06参照<br/>リリース開始解除指示は電源スイッチ短押しの想定|
|24|リリース中(DC-ON) → リリース待ち無し|リリース開始解除指示 or キャンセルコマンド受付|SysRS-03<br/>SysRS-16|SM-06参照<br/>リリース開始解除指示は電源スイッチ短押しの想定<br/>実際にあり得る遷移か要QA|
|25|リリース中(DC-ON) → リリース中(DC-OFF)|DC-OFF検知|SysRS-03|SM-06、SM-07参照<br/>実際にあり得る遷移か要QA|
|26|リリース中(DC-OFF) → 遠隔操作中|キャンセルコマンド受付 or 自動遷移|SysRS-03|SM-06、SM-07参照|
|27|DC-ON → 一時停止中|手動リリース|SysRS-16|SM-06、SM-07参照|
|28|遠隔操作中 → 一時停止中|一時停止操作|SysRS-06|SM-07参照<br/>一時停止操作は電源スイッチ短押しの想定|
|29|リリース中(DC-OFF) → 一時停止中|一時停止操作|SysRS-03<br/>SysRS-06|SM-07参照<br/>一時停止操作は電源スイッチ短押しの想定|
|30|一時停止中 → 遠隔操作中|一時停止解除操作|SysRS-07|SM-07参照<br/>一時停止解除操作は電源スイッチ短押しの想定|
|31|遠隔操作中 → ドッキング中|帰還コマンド受付|SysRS-03<br/>SysRS-06|SM-07参照|
|32|ドッキング中 → 遠隔操作中|キャンセルコマンド受付|SysRS-03|SM-07参照|
|33|ドッキング中 → 一時停止中|一時停止操作|SysRS-03<br/>SysRS-06|SM-07参照<br/>一時停止操作は電源スイッチ短押しの想定|
|34|充電中 → 充電完了|満充電検知|SysRS-16|SM-03、SM-06参照<br/>SM-06ではリリース待ち状態優先|
|35|充電完了 → 充電中|充電量低下検知|SysRS-16|SM-03、SM-06参照<br/>SM-06ではリリース待ち状態優先|
|36|Warning1 → Warning2|バッテリ残量回復検知|SysRS-10<br/>SysRS-11<br/>SysRS-16|SM-09参照|
|37|Warning2 → Warning1|バッテリ残量低下検知|SysRS-10<br/>SysRS-11<br/>SysRS-16|SM-09参照|
|38|一時停止中 → テスト|テスト開始(TODO)|SysRS-21|SM-01、SM-07参照<br/>遷移条件はSysRS-21で詳細化|
|39|テスト → 一時停止中|(TODO)|SysRS-21|SM-01、SM-07参照<br/>遷移条件はSysRS-21で詳細化|
|40|一時停止中 → リプロ|リプロ開始(TODO)|SysRS-22|SM-01、SM-07参照<br/>遷移条件はSysRS-22で詳細化|
|41|リプロ → 一時停止中|(TODO)|SysRS-22|SM-01、SM-07参照<br/>遷移条件はSysRS-22で詳細化|
...
- 備考
	...
	- 状態遷移分析による備考
      - 状態遷移No.1より<br/>電源OFF状態から充電ステーションに接続された場合は、Miimo Main ECUおよび表示(LED等)を起動する（通電中のLED表示する（起動中のLED表示しない））
      - 状態遷移No.6より<br/>通電中状態で、電源スイッチ短押し操作をされた場合は、頭部（首モータ含む）を起動する（起動中のLED表示する）
...
- 備考
	- 本ユースケース（UC）の実行中に、以下の各UCも実行される。詳細については各UCの記述を参照のこと
        ...
        - 異常を検知し停止する
	- 状態遷移分析による備考
      - 状態遷移No.18～23より<br/>DC-ON状態（充電ステーションに接続して頭部電源ONの状態）場合にて受け付けるリリースコマンドは、電源スイッチ短押し操作および頭部からのリリース指示の二つを受けてリリース動作を開始する（リリース中（DC-ON)状態に遷移する)とする
        - 上記電源スイッチ短押し操作、頭部からのリリース指示の一方を受けた場合、リリース待ち状態（LED表示はリリース待ち）になる
        - 上記電源スイッチ短押し操作によるリリース待ち状態は再度電源スイッチ短押し操作によりリリース待ち無し状態（LED表示は充電中か充電完了）に戻る
        - 上記電源スイッチ短押し操作によるリリース待ち状態はタイムアウトによりリリース待ち無し状態（LED表示は充電中か充電完了）に戻る
        - 上記頭部からのリリース指示によるリリース待ち状態はキャンセルコマンドによりリリース待ち無し状態（LED表示は充電中か充電完了）に戻る（タイムアウトはしない）
      - 状態遷移No.24～26より<br/>リリース動作中のキャンセルコマンド/電源スイッチ短押し操作は、<br/>・リリース動作中でDC-ON（リリース中(DC-ON)状態）の場合、リリース待ち無し状態に戻る。<br/>・リリース動作中でDC-OFF（リリース中(DC-OFF)状態）の場合、一時停止中状態となる。
...
- 備考
	- 状態遷移分析による備考
      - 状態遷移No.28～29、31、33より<br/>DC-OFF状態（充電ステーションから離れている電源ON状態）場合にて受け付ける電源スイッチ短押し操作は、一時停止中状態になるものとする。
...
- 備考
	...
	- 状態遷移分析による備考
      - 状態遷移No.8、11より<br/>バッテリ残量低下による警告（Warning1状態）は充電ステーション接続により解消される（電源スイッチ短押し操作せずLED表示が変わる）
      - 状態遷移No.36、37より<br/>・バッテリ残量低下による警告が解消されて他のWarningがあった場合は、他のWarningがあることをLED表示する。<br/>・他のWarningがありバッテリ残量低下による警告が発生した場合は、バッテリ残量低下による警告が発生したことをLED表示する。
...
- 備考
	- 状態遷移分析による備考
      - 状態遷移No.7、9、10、12、13より<br/>バッテリ残量低下以外の警告（Warning2状態）は電源スイッチ短押し操作により解消される（充電ステーション接続してもLED表示は変わらない）
      - 状態遷移No.14、15より<br/>・異常停止中の電源OFF操作は異常無しの電源OFF操作と変わらない挙動<br/>・充電ステーション接続中で頭部電源OFFでのFault状態は、手動リリース後の電源ON操作でのみ解消される（充電ステーション接続中で頭部電源OFFでのスイッチ長押し操作は無視される）。
      - 状態遷移No.36、37より<br/>・バッテリ残量低下による警告が解消されて他のWarningがあった場合は、他のWarningがあることをLED表示する。<br/>・他のWarningがありバッテリ残量低下による警告が発生した場合は、バッテリ残量低下による警告が発生したことをLED表示する。
...
- 備考
	- 状態遷移分析による備考
      - 状態遷移No.2より<br/>通電中（充電ステーション接続中で頭部電源OFF）状態で手動リリースされた場合、Miimo Main ECUを終了する（LED表示は終了中表示しない）
      - 状態遷移No.4、5より<br/>頭部電源ON状態での電源OFF操作は<br/>・充電ステーション未接続なら、電源OFFする（LED表示は終了中表示する）<br/>・充電ステーション接続中なら、頭部を電源OFF（シャットダウン処理）する（LED表示は終了中表示する）
      - 状態遷移No.14、15より<br/>上記異常無し状態の電源OFF操作と同様の挙動となる。そのため、充電ステーション接続中で頭部電源OFFでのFault状態は、手動リリース後の電源ON操作でのみ解消される（充電ステーション接続中で頭部電源OFFでのスイッチ長押し操作は無視される）。
...
- 備考
	- 状態遷移分析による備考
      - 状態遷移No.1より<br/>電源OFF状態から充電ステーションに接続された場合は、Miimo Main ECUおよび表示(LED等)を起動する（通電中のLED表示する（起動中のLED表示しない））
      - 状態遷移No.2より<br/>通電中（充電ステーション接続中で頭部電源OFF）状態で手動リリースされた場合、Miimo Main ECUを終了する（LED表示は終了中表示しない）
      - 状態遷移No.5より<br/>充電ステーション接続中の頭部電源ON状態での電源OFF操作は頭部を電源OFF（シャットダウン処理）する（LED表示は終了中表示する）
      - 状態遷移No.6より<br/>通電中状態で、電源スイッチ短押し操作をされた場合は、頭部（首モータ含む）を起動する（起動中のLED表示する）
      - 状態遷移No.15より<br/>・充電ステーション接続中で異常停止中の電源OFF操作は頭部電源ONでのみ有効。頭部電源OFFとなる。（充電ステーション接続中で頭部電源OFFでのスイッチ長押し操作は無視される）。
      - 状態遷移No.18～23より<br/>リリース待ち状態表示は、充電中・充電完了の表示よりも優先して表示される
...
|要求|備考|
|:---|:---|
|走行部は診断用PCからのログインに応じて通信を確立し、車体情報を送信する|車体情報はFrame numberを想定|
...
テレプレゼンスロボットは診断PCと接続・通信し、頭部・胴体部・走行部のモーター等動作確認等を行う
...
|頭部はソフトウェアバージョンを表示する||
|走行部は書き換えPCとの通信を確立し、車体情報を送信する|車体情報としてはFrame numberを想定|
