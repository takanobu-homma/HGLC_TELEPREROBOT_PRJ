...
|一時停止を解除する|電源スイッチ操作によりテレプレゼンスロボットを一時停止を解除する|・アバター設置者|REQ-024|
...
電源スイッチ操作によりテレプレゼンスロボットを一時停止を解除する
...
![](.images/conceptual/led_pattern_table.png)
...
|要求|備考|
|:---|:---|
|走行部は各部から状態の変化を受信しステータスを決定する|※１※２|
|胴体部はステータスを表示する|LEDランプを想定|

※１：ステータスの分類についてはL2レベルで詳細化する<br/>
※２：イベント（走行部に送られる状態の変化通知）は個別のフローを参照
