...
|ユースケース名|概要|アクター|関連要求|要求元システムUC|
|:---|:---|:---|:---|:---|
|起動する|テレプレゼンスロボットを起動し、各種操作が行えるようにする|・アバタークラウド<br/>・アバター設置者|REQ-001,<br/>REQ-006,<br/>REQ-007,<br/>REQ-008,<br/>REQ-032|UC-001,<br/>UC-003,<br/>UC-015|
|接続設定を行う|テレプレゼンスロボットをアバタークラウドに接続できるように設定する|・アバタークラウド|REQ-003|UC-001|
|遠隔で操作する|指定した操作コマンドによりテレプレゼンスロボットを遠隔操作する|・アバタークラウド|REQ-010,<br/>REQ-011,<br/> REQ-014,<br/>REQ-015,<br/>REQ-030|UC-005,<br/>UC-007,<br/>UC-014|
|遠隔で会話する|ユーザーがテレプレゼンスロボットを介して接触者と会話する|・アバタークラウド<br/>・接触者|REQ-012,<br/>REQ-013|UC-006|
|遠隔で設定する|遠隔でテレプレゼンスロボットのエリア検出方式などの設定を行う|・アバタークラウド|REQ-016|UC-008|
|一時停止する|ボタン操作によりテレプレゼンスロボットを一時停止する|・アバター設置者|REQ-022|UC-011|
|一時停止を解除する|ボタン操作によりテレプレゼンスロボットを一時停止を解除する|・アバター設置者|REQ-024|UC-011,<br/>UC-012|
|走行エリアを検知する|エリア信号を受信し走行エリアおよび充電ステーションを検出する|・充電ステーション|REQ-017,<br/>REQ-018,<br/>REQ-029|UC-008,<br/>UC-014|
|異常を検知し停止する|テレプレゼンスロボット内部の異常（バッテリー残量低下を含む）を検知し停止する|・アバタークラウド|REQ-020,<br/>REQ-026|UC-010,<br/>UC-012|
|状態を表示する|テレプレゼンスロボットの状態を本体のインジケーター（ステータスランプ）に表示する|・アバター設置者|REQ-005,<br/>REQ-023,<br/>REQ-025|UC-002,<br/>UC-011|
|状態を通知する|テレプレゼンスロボットの状態をアバタークラウドに通知する|・アバタークラウド|REQ-004,<br/>REQ-009,<br/>REQ-021,<br/>REQ-023,<br/>REQ-025,<br/>REQ-031,<br/>REQ-035|UC-002,<br/>UC-003,<br/>UC-010,<br/>UC-011,<br/>UC-012,<br/>UC-014,<br/>UC-017|
|データを送信する|バッテリ残量などのデータをアバタークラウドに送信する|・アバタークラウド|REQ-019|UC-002,<br/>UC-010|
|シャットダウンする|テレプレゼンスロボットの使用を終了し電源を切る|・アバター設置者|REQ-002,<br/>REQ-007,<br/>REQ-032|UC-001,<br/>UC-003,<br/>UC-004,<br/>UC-015|
|充電する|テレプレゼンスロボットを充電する|・充電ステーション|REQ-027,<br/>REQ-028|UC-013,<br/>UC-014|
|周囲の明るさを確認する|テレプレゼンスロボットの周囲の明るさを確認する||REQ-034|UC-017|
...
|走行部は頭部に電源を供給する||
|走行部はシステムの自己診断を行い結果を保存する||
|走行部は自己診断が正常に完了した場合、一時停止状態に設定する||
|走行部は自己診断で異常があった場合、結果の異常を知らせる|ステータスLEDなど|
