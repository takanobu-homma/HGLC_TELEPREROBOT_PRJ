...
ここで、状態間の遷移について、検討事項が無いか確認するため、
以下のような各状態とトリガによる状態遷移表を作成し、トリガごとに分析を実施した。

![](.images/statemachine/st_all_statemachine_chart.png)

<div style="page-break-before:always"></div>

上記分析を踏まえて、内部状態として、以下を取り上げる  
・State1内部状態  
・異常発生状態  
・State3内部状態  
・充電ステーション接続状態
...
|状態|定義|備考|
|:---|:---|:---|
|遠隔操作不可|State1において遠隔操作が出来ない状態|一時停止中を想定|
|遠隔操作可能|State1において遠隔操作可能な状態|さらに内部状態として、遠隔操作待ち、走行操作中、自律走行中を包含する|
|遠隔操作待ち|遠隔操作可能状態において、速度0により停止している状態||
|走行操作中|遠隔操作可能状態において、走行操作（速度0ではない）されている状態||
|自律走行中|遠隔操作可能状態において、自律走行している状態|自律走行としてはドッキング動作中とリリース動作中がある想定|

State1内部状態における内部状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR-01|遠隔操作不可 → 遠隔操作可能|遠隔操作許可操作|SysRS-03,SysRS-07|遠隔操作許可操作は電源スイッチ短押しを想定|
|TR-02|遠隔操作可能 → 遠隔操作不可|遠隔操作禁止操作|SysRS-03,SysRS-06|遠隔操作禁止操作は電源スイッチ短押しを想定|
|TR-03|遠隔操作待ち/走行操作中 → 走行操作中|走行操作指示|SysRS-03|走行操作指示はアバタークラウドからの前進/後進/旋回コマンドを想定|
|TR-04|走行操作中 → 遠隔操作待ち|速度0検知|SysRS-03||
|TR-05|遠隔操作待ち → 自律走行中|自律走行開始指示|SysRS-03|自律走行開始指示はアバタークラウドからの充電ステーションへの帰還コマンドを想定|
|TR-06|自律走行中 → 遠隔操作待ち|自律走行取消指示|SysRS-03|自律走行取消指示はアバタークラウドからのキャンセルコマンドを想定|
|TR-07|State3の自律走行中 → 自律走行中|充電ステーションとの接続断検知|SysRS-03|「State3内部状態」参照|
...
|状態|定義|備考|
|:---|:---|:---|
|Fault|State2、State4、State6、State8においてFaultレベルの異常が発生している状態||
|Warning1|State2、State6においてWarning1レベルの異常が発生している状態|充電ステーション接続中は発生しない想定|
|Warning2|State2、State4、State6、State8においてWarning2レベルの異常が発生している状態||
|Caution|State2、State4、State6、State8においてCautionレベルの異常が発生している状態||

異常発生状態における内部状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR-08|Fault/Warning1/Waring2/Caution → 退場点(State6へ)|電源OFF操作[State2]|SysRS-11a,SysRS-11b|※1|
|TR-09|Fault/Warning1/Waring2/Caution → 退場点(State8へ)|電源OFF操作[State4]|SysRS-11a,SysRS-11b|※1|
|TR-10|Warning1 → 退場点(State3へ)|ドッキング[State2]|SysRS-11a,SysRS-11b,SysRS-16|※2|
|TR-11|Warning1 → 退場点(State7へ)|ドッキング[State6]|SysRS-11a,SysRS-11b,SysRS-16|※2|
|TR-12|Warning1/Waring2/Caution → 退場点(State1へ)|異常復帰操作[State2]|SysRS-11a,SysRS-11b|※3|
|TR-13|Waring2/Caution → 退場点(State3へ)|異常復帰操作[State4]|SysRS-11a,SysRS-11b|※3 ※4|
|TR-14|Waring2/Caution → 退場点(State7へ)|異常復帰操作[State8]|SysRS-11a,SysRS-11b|※3 ※4|

※1:電源OFF操作は電源スイッチ長押し/シャットダウン操作を想定  
※2:Warning1発生時はドッキング操作が異常復帰操作を兼ねる想定  
※3:異常復帰操作は電源スイッチ短押しを想定。Fault状態は異常復帰操作を受け付けない想定  
※4:Warning1状態からの遷移はState2でのみ発生する想定  
...
|状態|定義|備考|
|:---|:---|:---|
|初期状態|State3において開始状態から遷移した状態|充電ステーション接続中状態の内部状態が状態表示される|
|リリース待ち|State3において自律走行の開始を待っている状態|自律走行は安全確認された後に開始する想定|
|自律走行中|State3において、自律走行している状態|自律走行としては充電ステーションからのリリースを想定|

State3内部状態における内部状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR-15|初期状態 → リリース待ち|自律走行開始指示|SysRS-03|自律走行開始指示はアバタークラウドからの充電ステーションからのリリースコマンドを想定|
|TR-16|リリース待ち → 初期状態|自律走行取消指示|SysRS-03|自律走行取消指示はアバタークラウドからのキャンセルコマンドを想定|
|TR-17|リリース待ち → 自律走行中|安全確認操作|SysRS-03|安全確認操作は電源スイッチ短押しを想定|
|TR-18|自律走行中 → 初期状態|自律走行取消指示、安全確認取消操作|SysRS-03|自律走行取消指示ははアバタークラウドからのキャンセルコマンドを想定、安全確認取消操作は電源スイッチ短押しを想定|
|TR-19|自律走行中 → State1の自律走行中|充電ステーションとの接続断検知|SysRS-03|「State1内部状態」参照|
...
|状態|定義|備考|
|:---|:---|:---|
|充電中|State3、State4、State7、State8においてバッテリ充電が実行されている状態|State3のリリース待ち/自律走行中、State4、State8の異常発生中においては、その状態表示が優先される|
|充電完了|State3、State4、State7、State8においてバッテリが満充電となり充電が実行されていない状態|State3のリリース待ち/自律走行中、State4、State8の異常発生中においては、その状態表示が優先される|

充電ステーション接続状態における内部状態遷移を以下に示す
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR-20|充電中 → 充電完了|満充電検知|SysRS-16|バッテリ残量100%の検知を想定|
|TR-21|充電完了 → 充電中|充電不足検知|SysRS-16|バッテリ残量閾値以下の検知を想定。閾値の具体的な値はTBD|
...
- 備考
	- 本ユースケース（UC）の実行中に、以下の各UCも実行される。詳細については各UCの記述を参照のこと
        ...
        - バッテリ残量低下を検知し停止する
        - 異常を検知し停止する
    - TR-15、TR-16、TR-17、TR-18、TR-19より
      - State3内部状態において、初期状態では自律走行開始指示（リリースコマンド）のみを受け付ける、受付後、リリース待ち状態（状態表示はリリース待ち）になる
      - リリース待ち状態では安全確認操作（電源スイッチ短押し）か自動走行取消指示（キャンセルコマンド）を受け付ける。
        - 安全確認操作を受付後、リリース動作を開始する（自律走行中状態に遷移する)とする
        - 自律走行取消指示を受付後、初期状態に戻る（状態表示は、充電中か充電完了）とする
      - 自律走行中状態では充電ステーション接続断が検知される以前に、自動走行取消指示または安全確認取消操作（電源スイッチ短押し）を受け付ける
        - 安全確認取消操作/自動走行取消指示を受付後、初期状態に戻る（状態表示は、充電中か充電完了）とする
        - 充電ステーション接続断が検知された後は、State1内部状態の自律走行中状態に遷移する
    - TR-03、TR-04、TR-05、TR-06、TR-07より
      - State1内部状態において、遠隔操作待ち状態では走行操作指示（走行操作コマンド）または自律走行開始指示（帰還コマンド）を受け付ける。
