...
|電源ON|テレプレゼンスロボットの電源が入っている状態|頭部・胴体部が起動している状態を想定|
|通電中|テレプレゼンスロボットに電源が入っていない状態（充電端子に接続されている）|充電中、充電完了の表示は行う想定|
|起動中|電源ON状態の中で起動処理を行っている状態||
|動作中|電源ON状態の中で起動処理が終わった状態|Fault/Warning/Caution状態を含む|
|正常動作中|動作中状態の中でFault/Warning/Caution状態ではない状態||
|Fault|動作中状態の中で内部デバイスの破損により走行ができない状態||
|Warning1|動作中状態の中で（TODO）|バッテリ残量低下|
|Warning2|動作中状態の中で（TODO）|CAN通信異常など|
|Caution|動作中状態の中で保護機能が動作し、走行動作を停止している状態||
|DC-ON|正常動作中状態の中で充電端子に接続されている状態||
...
|リリース中(DC-ON)|DC-ON状態の中でリリース動作を行っている状態||
|DC-OFF|正常動作中状態の中で充電端子に接続されていない状態||
|遠隔操作（外部指示）|DC-OFF状態の中で外部から遠隔操作が行える状態|頭部からCAN通信で走行部を制御できる状態を想定|
|一時停止|DC-OFF状態の中で（TODO）||
|ドッキング中|DC-OFF状態の中でドッキング指示によりドッキング動作を行っている状態||
|リリース中(DC-OFF)|DC-OFF状態の中でリリース動作を行っている状態||
|終了中|電源ON状態の中で終了処理を行っている状態||
|充電中|DC-ON状態または通電中状態の中でバッテリーを充電している状態||
|充電停止|DC-ON状態または通電中状態の中ででバッテリーの充電が完了している状態||
|異常|異常を検出している状態|Fault、Warning1、Warning2、Cautionを包含|
|リプロ|リプロモードで起動している状態||
|テスト|テストモードで起動している状態||
...
|---:|:---|:---|:---|:---|
|1|電源OFF → 通電中|充電ステーションに接続|SysRS-16||
|2|電源OFF → 電源ON|電源ON操作|SysRS-01|電源ON操作はスイッチ短押し、スイッチ長押し操作を想定|
|3|通電中 → 電源ON|電源ON操作|SysRS-01,SysRS-16|電源ON操作はスイッチ短押し操作のみを想定|
|4|電源ON → 電源OFF|電源OFF操作[充電端子未接続]|SysRS-15|電源OFF操作はシャットダウン操作、スイッチ長押し操作を想定|
|5|電源ON → 通電中|電源OFF操作[充電端子接続]|SysRS-15,SysRS-16|電源OFF操作はシャットダウン操作、スイッチ長押し操作を想定|
|6|正常動作中 → Fault|Fault検知|SysRS-11||
|7|正常動作中 → Warning|Warning検知|SysRS-11||
|8|正常動作中 → Caution|Caution検知|SysRS-11||
|9|DC-OFF → DC-ON|充電端子接続|SysRS-16||
|10|DC-ON → DC-OFF|充電端子接続解除|SysRS-16||
|11|XXX → YYY|TRG-01|SysRS-xxx||
