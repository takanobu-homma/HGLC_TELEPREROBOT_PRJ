...
|TR0-02|起動中/動作中/異常/テスト/リプロ → 電源OFF|電源OFF操作|SysRS-15[SysRS-11b or SysRS-19,SysRs-20,SysRS-21 or SysRs-22]|電源OFF操作はシャットダウン操作/電源スイッチ長押しを想定|
|TR0-03|起動中/動作中 → 異常|異常検知|SysRS-11a||
|TR0-04|異常 → 動作中|異常復帰操作|SysRS-11b|電源スイッチ短押しを想定。動作中遷移後はTR2-01/TR2-02で分岐する想定|
|TR0-05|※1 → テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|テストモード遷移指示は診断PCから送信される想定|
|TR0-06|※2 → リプロ|リプロモード遷移指示|SysRS-22|リプロモード遷移指示は書き換え用PCから送信される想定|
|TR0-07|起動中 → 動作中|起動中の完了による自動遷移|SysRS-01|動作中遷移後はTR2-01/TR2-02で分岐する想定|

※1:テストへは異常/動作中の一時停止/ステーション待機中のリリース待ち無しから遷移可能な想定。一時停止については、「SM-02:動作中」参照。リリース待ち無しについては、「SM-02-1:ステーション待機中」参照  
※2:リプロへは動作中の一時停止からのみ遷移可能な想定。一時停止については、「SM-02:動作中」参照
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR2-01|動作中の開始 → 一時停止|開始状態からの自動遷移[DC-OFF]|SysRS-01,SysRS-06|開始状態はTR0-04/TR0-07による遷移を想定|
|TR2-02|動作中の開始 → ステーション待機中|開始状態からの自動遷移[DC-ON]|SysRS-01|開始状態はTR0-04/TR0-07による遷移を想定|
...
|TR2-07|ドッキング移動中 → ステーション待機中|DC-ON検知|SysRS-03||
|TR2-08|ステーション待機中 → リリース移動中|DC-OFF検知|SysRS-03||
|TR2-09|リリース移動中 → 遠隔操作モード|ステーション離脱検知|SysRS-03|ステーション離脱検知はリリースの自律走行終了のタイミング※1|
|TR2-10|ステーション待機中 → 一時停止|手動リリース|SysRS-06|手押しで充電ステーションから切り離す操作※2|
|TR0-05|一時停止 → テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|※3|
|TR0-06|一時停止 → リプロ|リプロモード遷移指示|SysRS-22|※4|
...
※3:全体の状態遷移図のTR0-05として抽出済  
※4:全体の状態遷移図のTR0-06として抽出済
...
|No.|状態遷移|トリガー|影響を受ける要求|備考|
|---:|:---|:---|:---|:---|
|TR2-11|リリース待ち無し → リリース待ち|リリース指示|SysRS-03|リリース指示はアバターアプリから送信される想定|
|TR2-12|リリース待ち → リリース動作準備|安全確認操作|SysRS-03|安全確認操作は電源スイッチ短押しの想定|
|TR2-13|リリース待ち/リリース動作準備 → リリース待ち無し|キャンセル指示|SysRS-03|キャンセル指示はアバターアプリから送信される想定|
|TR0-05|リリース待ち無し → テスト|テストモード遷移指示|SysRS-19,SysRS-20,SysRS-21|※1|

※1:全体の状態遷移図のTR0-05として抽出済  

<div style="page-break-before:always"></div>

**SM-03:異常**

「異常」の内部状態を以下の通り抽出した（「電源OFF」の「状態表示」で異常がある場合も、本内部状態に従う）
...
※:異常時（「電源OFF」の「状態表示」での異常発生時は除く）でもテストモードの起動は可能な想定
...
|TR3-05|異常復帰操作可能 → 動作中|異常復帰操作|SysRS-11b|異常復帰操作は電源スイッチ短押しを想定※1※2|

※1:「電源OFF」の「状態表示」での異常では、異常復帰操作（電源スイッチ短押し）が電源ON操作となり、「起動中」へ遷移する想定  
※2:動作中への遷移としては全体の状態遷移図のTR0-04と同じ
...
- 備考
	- テレプレゼンスロボットを再起動してもFault状態が解消されない場合はメンテナンスが必要
    - TR0-02,TR2-01,TR2-02,TR0-04,TR3-05より
